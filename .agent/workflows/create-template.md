...
> ⚠️ **严重警告**：以下规则必须严格遵守，违反将导致 PPT 导出失败或显示异常。
...
> 📌 **记忆口诀**：PPT 只认基础形状 + 内联样式 + 系统字体 + 实色填充

---

## PPT 透明度兼容性规则（强制执行）

> ⚠️ **关键问题**：PPT 对 SVG 透明度支持有限，必须使用正确写法。

### 禁止的透明度写法

| ❌ 禁止                              | 问题描述                         | ✅ 正确替代方案                        |
| ------------------------------------ | -------------------------------- | -------------------------------------- |
| `opacity="0.3"` (元素透明度)         | PPT 可能不正确渲染               | 使用**浅色实色值**替代                 |
| `fill-opacity="0.3"`                 | PPT 支持有限                     | 使用**浅色实色值**替代                 |
| `fill="rgba(255,255,255,0.1)"`       | PPT 不支持 rgba 格式             | `fill="#FFFFFF" fill-opacity="0.1"`    |
| `stroke="rgba(0,0,0,0.5)"`           | PPT 不支持 rgba 格式             | `stroke="#000000" stroke-opacity="0.5"`|
| `<g opacity="0.2">...</g>`           | PPT 不支持组透明度               | 每个子元素单独设置透明度               |
| `<image ... opacity="0.3"/>`         | PPT 不支持图片透明度             | 用遮罩层 `<rect fill="背景色" opacity="0.7"/>` |
| `filter="url(#shadow)"`              | PPT 不支持 SVG 滤镜              | 移除滤镜效果                           |

### 透明度的正确替代方案

**推荐做法**：用**浅色实色值**代替透明度，确保 PPT 兼容。

```xml
<!-- ❌ 禁止：使用透明度 -->
<path fill="#DE3545" opacity="0.3"/>
<path fill="#DE3545" fill-opacity="0.4"/>

<!-- ✅ 正确：使用计算后的浅色实色 -->
<path fill="#F5A0A8"/>  <!-- 相当于 #DE3545 + 30% 透明度 -->
<path fill="#E97983"/>  <!-- 相当于 #DE3545 + 40% 透明度 -->
```
...
| 原色      | 透明度 | 替代实色  | 备注                |
| --------- | ------ | --------- | ------------------- |
| `#DE3545` | 30%    | `#F5A0A8` | 淡红色              |
| `#DE3545` | 40%    | `#E97983` | 浅红色              |
| `#DE3545` | 50%    | `#E86A76` | 中浅红色            |
| `#DE3545` | 60%    | `#E45967` | 中红色              |
| `#333333` | 30%    | `#ADADAD` | 淡灰色              |
| `#333333` | 40%    | `#999999` | 浅灰色              |
| `#333333` | 50%    | `#666666` | 中灰色              |
| `#FFFFFF` | 30%    | `#F5F5F5` | 近白色（浅灰背景）  |

> 💡 **计算公式**：混合色 = 原色 × 透明度 + 背景色 × (1 - 透明度)
...
<defs>
  <marker id="arrow" markerWidth="10" markerHeight="7" refX="9" refY="3.5" orient="auto">
    <polygon points="0 0, 10 3.5, 0 7" fill="#DE3545"/>
  </marker>
</defs>
<path d="M100 200 L200 200" marker-end="url(#arrow)"/>

<!-- ✅ 正确：使用 line + polygon -->
<line x1="100" y1="200" x2="195" y2="200" stroke="#DE3545" stroke-width="2"/>
<polygon points="195,194 210,200 195,206" fill="#DE3545"/>
```
...
- [ ] 无 `mask`
- [ ] 无 `<style>` 标签
- [ ] 无 `class` 或 `id` 选择器（用于样式）
- [ ] 无外部 CSS 引用
- [ ] 无 `<foreignObject>`
- [ ] 无 `textPath`
- [ ] 无 `@font-face` 或 Web 字体
...
- [ ] 无 `rgba()` 颜色格式
- [ ] 无 `<g opacity="...">` 组透明度
- [ ] 无 `<image ... opacity="..."/>` 图片透明度
- [ ] 所有透明效果已替换为**浅色实色值**
- [ ] 颜色值统一使用 HEX 格式

### 格式规范检查

- [ ] viewBox 设置正确（`0 0 1280 720`）
- [ ] 占位符格式一致（`{{PLACEHOLDER}}`）
- [ ] 设计风格在所有页面中保持一致
