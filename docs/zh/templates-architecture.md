...
每个 index 跟物理目录一一对应，字段按需精简，沿用 Visualization family 索引（[Chart](../../skills/ppt-master/templates/charts/charts_index.json)、[Structure](../../skills/ppt-master/templates/structures/structures_index.json)、[Table](../../skills/ppt-master/templates/tables/tables_index.json)）的紧凑“meta + summary”模式，同时保留对 Strategist 选型有用的结构化元数据。
