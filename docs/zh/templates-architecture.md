...
| 范围 | 工作区根目录 | 核心结构 | 发现行为 |
|---|---|---|---|
| `library`（默认） | `skills/ppt-master/templates/<kind>/<id>/` | `templates/` + `images/` + `icons/` + `exports/` | 写入对应全局索引 |
| `project` | `projects/<name>/` | 完全相同的 `templates/` + `images/` + `icons/` + `exports/` 契约 | 不更新全局索引 |
...
├── images/                     # 所有位图；SVG 统一引用 ../images/<name>
├── icons/                      # 提取向量素材的运行期副本
└── exports/
    └── <id>_template_preview.pptx
```

预览 PPTX 是派生的审阅证据，不是模板源资产。Step 3 读取工作区根目录，只消费 `templates/`、`images/` 与 `icons/`，不会复制或使用 `exports/`。

两种范围都在可移植 frontmatter 中保留 `kind: layout` 或 `kind: deck`。`output_scope` 与 `target_project` 只属于工作流简报，不写入 `design_spec.md`。

任何范围第一次写最终文件前，都必须解析工作区根目录、确认 `templates/` 为空，并检查全部计划写入的图片、图标与预览 PPTX 文件名无冲突。项目范围还必须确认目标项目已初始化。任一失败都在写入前停止，不合并、不覆盖。
...
| `workflows/create-template.md` | 完整 layout 或 deck 工作区。输出范围默认 `library`（`templates/<kind>/<id>/` + 注册），确认 `project` 时写 `projects/<name>/`（不注册）。两者具有相同核心目录与预览 PPTX。内部 kind 分支仍默认 deck；用户明说"只要结构 / 丢掉品牌色"时走 layout |
