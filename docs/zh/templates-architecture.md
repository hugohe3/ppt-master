...
**触发规则仍以路径为准**——仍需显式目录路径（见 [[feedback-template-explicit-path-only]]），裸名称绝不触发。复制 Layout/Deck 包之前，Step 3 会验证当前 structured 合同；旧 `native_structure_mode: template`、缺 Master 身份、原子 placeholder 或蒸馏时代的包必须先运行 `restore-pptx-structure`。唯一的窄例外是当前对话刚完成项目范围 `create-template`：验证通过后可把精确的 `<project>/templates/` 输出直接交给 Step 3。`kind` 字段决定**触发后 AI 怎么处理**：
