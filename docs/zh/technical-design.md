...
**为什么原生对象重建使用 marker，而不是自动替换对象。** 独立的 `pptx_to_svg.py` 导入器会为受支持的未合并表格和保守 classic-chart cache 输出 SVG fallback，并附加 `data-pptx-native` 元数据。表格样式 `{5C22544A-7EE6-4342-B048-85BDC9FD1C3A}` 的规范化 fallback 会解析 `wholeTbl`、`firstRow`、横向带状行、主题颜色 / 字体和直接格式覆盖；这不代表已经实现完整 built-in/custom style registry。受支持的柱 / 条 / 折线 / 面积、饼 / 圆环、散点 / 气泡图在没有 baked preview 时会生成确定性、可读的 SVG fallback，并标记 `visual=normalized`。常见 solid/no-fill/line/marker 写法和 scheme color 会被保守解析，未知 style XML 仍 fail closed；payload 未建模的样式细节在 native 替换时仍可能归一化，并由 editable-first warning 明示。图表标题、图例、坐标轴标题和受支持的数据标签语义继续保留。默认导出把 fallback 子元素当普通 DrawingML shape 处理；只有 `--native-objects` 才启用可编辑的 PowerPoint 表格 / 图表。合并单元格、直接边框、非纯色填充、混合富文本等不支持的表格场景继续保留 SVG table。ChartEx 导入、旋转 / 翻转 / 3D / 组合图表、不受支持的数据标签、自定义坐标轴、趋势线 / 误差线、不受支持的子类型、稀疏或非有限 cache、日期 / 格式化类别轴等场景在有 baked preview 时保留预览。当前 renderer 范围之外但 payload 仍有效的类型（包括 `of_pie`）继续使用 `visual=placeholder` / `route=reconstruction-only`，native opt-in 仍可重建。每个 active imported marker 都写入 `data-pptx-fallback-sha256`：可见 fallback、可达的 SVG fragment definition、marker 内部引用目标或 marker transform 后续被改动时，强制质量检查器会报告 stale；默认导出继续保留 SVG，native replacement hard fail。无 hash 的旧 marker 仍兼容，只在 checker/native 路线提示无法检测 stale。ChartEx emitter 现在把有效 payload colors 写入 color-style part，但不承诺完整源样式保真。非法状态声明继续作为 error。这对 importer/exporter 是重建辅助能力，不能替代 `template-fill-pptx` 或 `native-enhance-pptx` 的保留型路线。
