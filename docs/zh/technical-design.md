...
**为什么默认 native deck 使用基线母版，而不推断模板。** `svg_to_pptx.py` 会保留基础 package 中空白版式的真实关系，并默认使用 `--pptx-structure baseline`。Baseline 只提升低风险共享结构：严格多数页面完全一致的页面背景（每页都必须携带显式背景，少数派页面保留自己的背景并覆盖母版填充），以及位于 shape 序列开头（视觉后层）、且 id 精确 token 为 `logo`、`footer`、`header`、`watermark`、`chrome`、`pageNumber` 或 `slideNumber` 的重复 SVG chrome。Chrome 还必须在共享同一母版的严格多数页面上生成完全一致的 OOXML、没有被页面动画引用，并保持相同的前置 z-order；覆盖在正文之上的 chrome 继续留在 slide 本地。少数派页面（封面、章节页）的全部元素保持 slide-local，并被改绑到生成的 `Cover` 版式（`showMasterSp="0"`）——遵循 PowerPoint 的惯例：页型差异用同一母版下的版式表达而不是增加母版，因此被提升的母版 chrome 绝不会泄漏到这些页面上。被提升 chrome 的图片关系会复制到 slide master，基础模板中没有任何页面引用的 slide layout 也会从 package 中裁剪掉；`pageNumber`/`slideNumber` chrome 的文本若恰好等于该页应显示的页码（尊重 `firstSlideNum`），会转换为自动更新的 PowerPoint 页码字段，此后在 PowerPoint 中调整页序页码自动重算，其他编号方案保持字面文本不动。这样普通 deck 能获得真实的 master/layout 结构，同时不根据视觉相似度猜测可复用版式或占位符。
