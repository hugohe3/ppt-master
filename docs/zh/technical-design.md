...
| 用户想从 PPTX 或设计参考构建可复用模板工作区 | `create-template` 或 `create-brand` | 输出后续能触发 Step 3 的工作区根目录；create-template 还会导出审阅 PPTX |
| 用户提供明确模板路径 | 主 SVG 流水线 Step 3 | 当前 Layout/Deck 工作区解析 `templates/design_spec.md`；Brand 与旧式平铺包解析根目录 `design_spec.md`；只有旧 SVG 语义才恢复结构 |
...
**机械触发，不做语义匹配。** 像 `academic_defense` 这样的裸名字、品牌提及，或“麦肯锡风格”这类风格短语，即使库里存在相似目录，也不会触发 Step 3。Step 3 只消费显式路径。当前 Layout/Deck 工作区解析 `templates/design_spec.md`；Brand 包维持根目录 `design_spec.md` 合同，兼容的旧式平铺 Layout/Deck 包也从根目录读取。目录平铺本身不是恢复理由；只有旧 Master/Layout/placeholder 语义才触发 `restore-pptx-structure`。发现性交给模板索引和显式问答（“有哪些模板可以用？”），不交给运行时 fuzzy matching。

新建 Layout/Deck 在两种位置都采用同一物理合同：

```text
<template_workspace>/
├── templates/   # design_spec.md、SVG 原型，以及使用时的 templates/icons/
├── images/      # 位图素材，SVG 统一引用 ../images/<name>
├── icons/       # 提取向量素材的运行期副本
└── exports/     # <id>_template_preview.pptx
```

`<template_workspace>` 可以是 `skills/ppt-master/templates/<kind>/<id>/`，也可以是 `projects/<name>/`。Step 3 接收这个根目录。工作区可在两个位置之间迁移而不改形；唯一的范围差异是全局索引注册，`exports/` 只作为审阅证据，不参与模板输入。
...
**为什么只有一条 PPTX 编译路线。** Native 导出把作者 SVG 中受支持的元素逐个翻译成 DrawingML 形状。常规 deck 路线读取 `svg_output/`；create-template 对通过校验的模板原型调用同一 structured 编译器，生成 `exports/<id>_template_preview.pptx` 作为审阅证据。项目不会把整页 SVG 媒体或另一套位图渲染打包成第二类 PPTX。`svg_final/` 仍由常规 deck 的强制后处理生成，但只承担自包含视觉预览和 SVG 图片插入，不为 PowerPoint 手工“转换为形状”提供兼容兜底。
...
**为什么 strict 与 adaptive 模板共享一条 structured 路线。** `page_layouts` 记录完整输入原型，`pptx_masters` 与 `pptx_layouts` 从规划阶段起记录输出归属。strict 保持原型合同；adaptive 保持原型 Master，只有固定 Layout 原子或槽位 topology/bounds 改变时才使用新 Layout key，并在页面创作时立即更新 mapping，而不是事后推断。非 mirror 的皮肤由项目控制；mirror 保持复用视觉身份，但来源 Master/Layout group 仍会按合同原子化。
...
**为什么可复用模板要重建来源拓扑。** `pptx_template_import.py` 输出分层 Master/Layout/Slide 参考和 native 结构事实；`create-template` 重建每一个来源 Master 和 Layout，包括未使用 Layout，并生成完整注释 SVG 原型。来源 Master/Layout group 被原子化；父子关系、选择器名称、placeholder 身份和 bounds 在模板规格中保持可审计。原 PPTX 只是分析输入，不再成为打包依赖。

**为什么 create-template 在两种范围都使用同一工作区形态。** `create-template` 仍以写入索引的 `library` 为默认，也可写入已初始化项目。两种根目录都包含相同的 `templates/`、`images/`、`icons/` 与 `exports/` 核心结构，包括 `<id>_template_preview.pptx`；SVG 素材引用也完全一致。因此完整工作区可直接迁移和复用，不需要全局库专用 package 分支或缩减的项目分支。唯一范围差异是全局索引注册，两种范围都使用同一 `structured` 合同。
...
| `create-template` | 构建可复用 layout/deck 模板工作区 | 输出可移植工作区根目录与 `exports/<id>_template_preview.pptx`；只有全局库范围执行注册 |
