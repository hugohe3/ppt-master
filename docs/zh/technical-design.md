...
**为什么默认 native deck 使用基线母版，而不推断模板。** `svg_to_pptx.py` 会保留基础 package 中空白版式的真实关系，并默认使用 `--pptx-structure baseline`。Baseline 只提升低风险共享结构：完全一致的页面背景，以及各页共同位于 shape 序列开头（视觉后层）、且 id 精确 token 为 `logo`、`footer`、`header`、`watermark`、`chrome`、`pageNumber` 或 `slideNumber` 的重复 SVG chrome。Chrome 还必须在共享同一母版的所有页面上生成完全一致的 OOXML、没有被页面动画引用，并保持相同的前置 z-order；覆盖在正文之上的 chrome 继续留在 slide 本地。被提升 chrome 的图片关系会复制到 slide master。这样普通 deck 能获得真实的 master/layout 结构，同时不根据视觉相似度猜测可复用版式或占位符。

**为什么原生对象重建使用 marker，而不是自动替换对象。** 独立的 `pptx_to_svg.py` 导入器会为受支持的未合并表格和保守 classic-chart cache 输出 SVG fallback，并附加 `data-pptx-native` 元数据；当现有 schema 能表达时，还会保留源表格样式继承、受支持的单元格纯色填充 / 基础文字格式，以及图表标题、图例、坐标轴标题和面积图 / 条形图 / 柱形图 / 折线图的 plot 级数据标签开关。默认导出仍把 fallback 子元素当普通 DrawingML shape 处理；只有 `--native-objects` 才会启用可编辑的 PowerPoint 表格 / 图表。合并单元格、直接边框 / 非纯色填充 / 混合富文本、ChartEx、带变换 / 3D / 组合图表、不受支持的数据标签作用域或类型、自定义坐标轴语义、趋势线 / 误差线、不受支持的子类型参数、稀疏或非有限 cache、日期 / 格式化类别轴等不支持场景，会在有 baked preview 时保留预览，否则给出明确占位符，并写入 `data-pptx-native-status`。质量检查器和 native-object 导出会把这些 fallback-only 对象报告为 warning。这对 importer/exporter 是重建辅助能力，不能替代 `template-fill-pptx` 或 `native-enhance-pptx` 的保留型路线。
