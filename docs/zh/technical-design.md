...
**模板合同是 opt-in，Visualization 参考和图标不是。** 这种不对称是有意的：Layout 锁定可复用结构，Style 锁定方向/方法默认值，Brand 锁定身份，Deck 则把重复应用与身份、结构组合起来；页面级 Chart、Structure、Table 参考和图标只是不会单独施加 deck 级合同的复用原语。同一个 `templates/` 目录，但 ownership 不同。
