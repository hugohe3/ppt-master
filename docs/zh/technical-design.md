...
**为什么原生对象重建使用 marker，而不是自动替换对象。** 独立的 `pptx_to_svg.py` 导入器会为已验证的纯文本表格和保守 classic-chart cache 输出 SVG fallback，并附加 `data-pptx-native` 元数据。表格子集现已覆盖未合并网格、slave 为空的规范矩形 merge topology、安全的 solid/no-fill 逐边 border，以及 run 样式一致的纯文本多段落单元格；非规范或重叠 merge、非空 slave、不安全 border XML、非纯色填充和混合 run-level 富文本仍 fail closed。表格样式 `{5C22544A-7EE6-4342-B048-85BDC9FD1C3A}` 的规范化 fallback 会解析 `wholeTbl`、`firstRow`、横向带状行、主题颜色 / 字体和直接格式覆盖；这不代表已经实现完整 built-in/custom style registry。受支持的柱 / 条 / 折线 / 面积、饼 / 圆环、散点 / 气泡图在没有 baked preview 时会生成确定性、可读的 SVG fallback，并标记 `visual=normalized`。导入器还覆盖已验证的柱 / 折线 / 面积组合图子集、规范四系列 OHLC stock，以及使用数值日期轴的面积图；组合图的主 / 次 plot 可保留独立 category cache 和 workbook range。坐标轴保留只采用封闭的 category/value 角色与 kind、position、visibility、label position、number format、min/max/major unit、reverse、major_gridlines 字段，不是完整 `AxisSpec`。常见 solid/no-fill/line/marker 写法和 scheme color 会被保守解析，显式归一化边界之外的未知 series paint/style XML 仍 fail closed。安全的 stock series style 可通过结构门禁，但 stock series、`hiLowLines` 与 up-down bar 的局部样式仍可能按 editable-first 合同归一化。payload 未建模的样式细节在 native 替换时仍可能归一化，并由 editable-first warning 明示。图表标题、图例、坐标轴标题和受支持的数据标签语义继续保留。默认导出把 fallback 子元素当普通 DrawingML shape 处理；只有 `--native-objects` 才启用可编辑的 PowerPoint 表格 / 图表。ChartEx 导入、旋转 / 翻转 / 3D 图表、已验证子集之外的 combo/stock 变体、其他日期轴图表族、未建模坐标轴语义、不受支持的数据标签、趋势线 / 误差线、不受支持的子类型、稀疏或非有限 cache 等场景在有 baked preview 时继续保留预览。当前 renderer 范围之外但 payload 仍有效的类型（包括 `of_pie`、combo 和 stock）在没有源 preview 时继续使用 `visual=placeholder` / `route=reconstruction-only`，native opt-in 仍可重建。每个 active imported marker 都写入 `data-pptx-fallback-sha256`：可见 fallback、可达的 SVG fragment definition、marker 内部引用目标或 marker transform 后续被改动时，强制质量检查器会报告 stale；默认导出继续保留 SVG，native replacement hard fail。无 hash 的旧 marker 仍兼容，只在 checker/native 路线提示无法检测 stale。ChartEx emitter 现在把有效 payload colors 写入 color-style part，但不承诺 ChartEx 导入或完整源样式保真。非法状态声明继续作为 error。这对 importer/exporter 是重建辅助能力，不能替代 `template-fill-pptx` 或 `native-enhance-pptx` 的保留型路线。
