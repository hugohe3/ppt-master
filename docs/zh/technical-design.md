...
[Executor] 执行师
    ...
    ├── [Quality Check] svg_quality_checker.py（强制通过，0 错误；warning 非阻塞）
...
| 用户想从 PPTX 或设计参考构建可复用模板工作区 | Create Template（`create-template`） | 固定入口只分派一个 Create Brand、Create Layout 或 Create Deck 子工作流；所得工作区根目录后续触发 Generate Step 3，结构型子工作流可导出审阅 PPTX |
...
常见失败的停 / 继续规则以 [`failure-recovery.md`](../../skills/ppt-master/workflows/governance/failure-recovery.md) 为准；本节不复制恢复矩阵。
...
| 治理文档 | `failure-recovery` | Generate PPTX 各阶段的停止 / 继续决策 |

这种分类是职责边界，不是文件命名偏好。只有出现不同的产物生命周期和修改模型时，才新增顶层路线；Create Template 内按模板类型区分的执行归入子工作流，生成路线内的可选行为归入 profile、stage 或 governance。
