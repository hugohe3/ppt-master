...
| 用户提供明确的 `templates/.../<id>/` 目录且声明 `kind: brand/layout/deck` | 主 SVG 流水线 Step 3 | Deck/Layout SVG 必须满足当前 structured 合同；旧包先运行 `restore-pptx-structure` |
...
**为什么项目本地薄模板只是输出政策，不是另一种结构模式。** `create-template` 仍以写入索引的 `library` 为默认，也可把同一份已校验合同直接写入已初始化项目的 `templates/` 根目录。项目位图进入 `images/`，抽取图标保留 package 副本与运行时副本，且不修改全局索引。两种范围都使用同一 `structured` 合同。
