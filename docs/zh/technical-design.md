...
**为什么导入的可复用模板应保留而不是重建。** `pptx_template_import.py` 会配对输出 `native_structure.json` 与 `source_template.pptx`。当 `create-template` 确认源 master/layout 图谱可复用后，`pptx_structure.mode: preserve` 以该 package 为底包，把生成页绑定到精确的源 layout part，移除 SVG 中仅用于预览的继承层副本，并恢复源 placeholder 的 type/idx 身份。源文件已有的多母版和未被示例页引用的版式也会继续保留在 PowerPoint 的版式选择器中。原始 PPTX 加一次性新内容仍走 `template-fill-pptx`；preserve 模式只从完成封装的可复用模板包开始。

**为什么每个 SVG 应引用版式，而不是直接变成版式。** 生成 SVG 是完整的 slide 实例；其中真实标题、正文、图片、表格和图表必须继续能在普通视图中选择。`spec_lock.md` 因而把 `pptx_layouts`（每页最终引用的 PowerPoint layout key/name）与 `page_layouts`（可选的输入 SVG 设计参考）分开。只有静态 layout layer 与 placeholder contract 相同的页面才复用同一个输出 key。没有 CLI 覆盖时，导出器会读取 lock 中的 `pptx_structure.mode`。在这个继承层级里，归属由复用范围而不是填充类型决定：全画布纯色矩形在全 deck 共享时成为 Master `p:bg`，页型共享时成为 Layout `p:bg`，显式标记为 slide 或位于背景层但未标记时成为 Slide `p:bg`。这样新增页面能继承正确背景，同时不会把实际内容或可选中的全屏矩形塞进版式。
