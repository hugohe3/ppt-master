...
**策略师确认阶段是唯一的阻塞 gate。** Strategist 阶段以一个三阶段确认 gate 作为核心阻塞决策点：第一阶段确认方向锚点（画布、受众、改写幅度、交付目的、mode、visual style，以及 Step 3 加载 Deck/Layout 模板时出现的 `template_reuse_scope`；`template_adherence` 只在 `mirror` / `layout` 下出现）；第二阶段基于已确认锚点重新推导并确认设计系统（页数、调色板、字体、图标、公式策略）；第三阶段基于已确认设计系统重新推导并确认图片与执行方式（图片来源、生成图风格、AI 图片路径、生成模式、refine-spec）。复用范围默认推荐 `layout`；`style` 选择 flat 自由设计路线，structured 的 `mirror` / `layout` 默认推荐 `adaptive`，允许无匹配构图时在同一 Master 下创建新 Layout；`strict` 则保持所选 Layout 契约不变。最终的 `confirm_ui/result.json` 是权威输入。
