...
**为什么默认 native deck 使用基线母版，而不推断模板。** `svg_to_pptx.py` 会从基础 package 中空白版式的真实关系开始构建，并默认使用 `--pptx-structure baseline`。Baseline 只提升低风险共享结构：严格多数页面完全一致的页面背景（每页都必须携带显式背景，少数派页面保留自己的背景并覆盖母版填充），以及位于 shape 序列开头（视觉后层）、且 id 精确 token 为 `logo`、`footer`、`header`、`watermark`、`chrome`、`pageNumber` 或 `slideNumber` 的重复 SVG chrome。Chrome 还必须在共享同一母版的严格多数页面上生成完全一致的 OOXML、没有被页面动画引用，并保持相同的前置 z-order；覆盖在正文之上的 chrome 继续留在 slide 本地。少数派页面（封面、章节页）的全部元素保持 slide-local，并被改绑到生成的 `Cover` 版式（`showMasterSp="0"`）——遵循 PowerPoint 的惯例：页型差异用同一母版下的版式表达而不是增加母版，因此被提升的母版 chrome 绝不会泄漏到这些页面上。全部页面生成完成后，导出器只根据文件名中的明确角色 token 分配 `Cover`、`Agenda`、`Section`、`Closing` 或兜底 `Content`；它保留当前版式的母版图形可见性，不比较页面视觉、不猜占位符，只有某一家族的每页都携带完全相同的显式 `p:bg` 时才把该背景提升到 Layout，实际页面内容始终留在 Slide。被提升 chrome 的图片关系会复制到 slide master，基础模板中没有任何页面引用的 slide layout 也会从 package 中裁剪掉；`pageNumber`/`slideNumber` chrome 的文本若恰好等于该页应显示的页码（尊重 `firstSlideNum`），会转换为自动更新的 PowerPoint 页码字段，此后在 PowerPoint 中调整页序页码自动重算，其他编号方案保持字面文本不动。这样普通 deck 能获得实用的 master/layout 清单，同时不限制 SVG 创作，也不消耗模型上下文预先规划版式。
...
**为什么导入的可复用模板应保留原生结构但不强制使用。** `pptx_template_import.py` 会配对输出 `native_structure.json` 与 `source_template.pptx`。当源 master/layout 图谱可复用时，`create-template` 可以在照常创建 SVG 模板 roster 的同时保留这对文件。后续 Strategist 确认模板遵循方式：`strict` 可以启用 `pptx_structure.mode: preserve`，以源 package 为底包，把每个生成页绑定到精确的源 layout part，移除 SVG 中仅用于预览的继承层副本，并恢复源 placeholder 的 type/idx 身份；`adaptive` 则把模板 SVG 当作可选参考，原生导出仍走 baseline。只有严格 preserve 路径会把源文件已有的多母版和未被示例页引用的版式继续保留在 PowerPoint 的版式选择器中。原始 PPTX 加一次性新内容仍走 `template-fill-pptx`；preserve 模式只从完成封装的可复用模板包开始。

**为什么每个 SVG 应引用版式，而不是直接变成版式。** 生成 SVG 是完整的 slide 实例；其中真实标题、正文、图片、表格和图表必须继续能在普通视图中选择。`spec_lock.md` 因而把 `pptx_layouts`（结构化页面最终引用的 PowerPoint layout key/name）与 `page_layouts`（输入 SVG 设计参考）分开。确认 `template_adherence: strict` 时，每页都必须选择一个模板 SVG，并可进一步启用原生版式保留；确认 `adaptive` 时，允许省略 `page_layouts` 让页面自由设计，原生导出继续走 baseline。只有静态 layout layer 与 placeholder contract 相同的页面才复用同一个输出 key。没有 CLI 覆盖时，导出器会读取 lock 中的 `pptx_structure.mode`。在这个继承层级里，归属由复用范围而不是填充类型决定：全画布纯色矩形在全 deck 共享时成为 Master `p:bg`，页型共享时成为 Layout `p:bg`，显式标记为 slide 或位于背景层但未标记时成为 Slide `p:bg`。这样新增页面能继承正确背景，同时不会把实际内容或可选中的全屏矩形塞进版式。
