...
| 导入的文字 run 效果 | 逻辑 shape 上未变更的 `metadata[data-pptx-part="txbody"]`；独立竖排输出使用仅限导入的阻塞效果状态 | `p:txBody` 内原始原生 run 效果 | 仅在原始 payload 仍可用时为 `Native-stable`；会丢失非空 run `effectLst` / `effectDag` 的编辑或竖排文本规范化将被阻断 | 不是公开创作语法；含关系引用的文本内容及表格单元格文本尚不在该保护范围内 |
