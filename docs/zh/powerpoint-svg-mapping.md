...
| 图片阴影或发光 | 未裁剪 `<image>` 直接承载一个已登记效果 filter；裁剪图片或导入 crop 使用精确单图片外层载体 | 原生 `p:pic/p:spPr/a:effectLst` | `Approximate`；单一效果以一个可编辑图片往返 | `filter` 与 `clip-path` 不得同挂一个 `<image>`；Layer/Placeholder/Binding/替换所有权留在载体外层；普通组 filter、其他 primitive、效果 DAG 与多个独立效果仍不支持 |
