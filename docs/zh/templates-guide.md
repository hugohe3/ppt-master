...
| **原生结构策略** | 有完整 master/layout 时推荐 `preserve`（保留原 package）；结构薄弱时用 `template`（重建一个 master + 语义版式）。`mirror` 因为继承层已展平，只走重建/基线路线。 |
...
- **模板不一定等于母版，但可以携带母版**。普通模板仍以 SVG 骨架为主；带 `native_structure.json` + `source_template.pptx` 的导入模板会在导出时保留原 master/layout/theme，并把生成内容绑定到原 placeholder
