...
PPT Master 的"模板"是一份**结构 + 风格**的预设包：包含若干页面布局 SVG（封面/章节/目录/内容/结尾及其变体）、`design_spec.md` 设计规范，以及配套素材（logo、背景、装饰图）。从结构完整的 PPTX 导入时，它还可以携带原生 master/layout 契约和源模板 package；没有这对文件时，它仍是由工作流复用的 SVG 页面骨架。
...
**强烈推荐：直接给原始 `.pptx` 文件。** 当前的 PPTX 导入管线会用 [`pptx_template_import.py`](../../skills/ppt-master/scripts/pptx_template_import.py) 直接读取 OOXML，提取主题色、字体、全部 master/layout、placeholder 元数据和可复用图片资源，同时输出 `native_structure.json` 与字节保留的 `source_template.pptx`。结构完整时，模板包会保留后续生成页直接引用原版式的能力；结构薄弱时，Template_Designer 才会基于 layered `svg/` 重建干净的 master/layout 规范。保留原生结构不等于强制未来每份 deck 都使用它。
...
| **原生结构封装策略** | 有完整 master/layout 时推荐 `preserve`（保留原 package）；结构薄弱时用 `template`（重建一个 master + 语义版式）。它决定模板库资产保留什么能力，不决定未来每份 deck 必须怎样使用。`mirror` 因为继承层已展平，只走重建/基线路线。 |
...
- **适应性使用（adaptive）**——只使用适合的 SVG 模板页，不适配的页面允许自由设计，原生导出继续走普通 baseline
- **严格套用（strict）**——每一页都映射到模板 roster；模板携带兼容原生结构时，复用原始 Master/Layout/Placeholder
...
- **模板不一定等于母版，但可以携带母版**。普通模板仍以 SVG 骨架为主；带 `native_structure.json` + `source_template.pptx` 的导入模板会把原 master/layout/theme 保留为严格套用时的可选能力，适应性使用仍走普通 SVG + baseline 路径
