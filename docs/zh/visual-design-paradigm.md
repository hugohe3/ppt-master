...
> 这份文档是对 [`executor-base.md`](../../skills/ppt-master/references/executor-base.md) §3 的深度升级，融合了八个维度的工艺研究与三轮专家评审结论。目标：让 Executor 在手写 SVG 时具备「以什么标准判断每个决策对不对」的内化能力。
...
## 1. 自检视角（高级 ↔ 廉价的几个维度 — 反思用，不是评分闸门）

起草后用下面几维回看自己的草稿——每维「0 分端」是廉价/线框图、「10 分端」是高级/定制，作为**反思的两个锚点**。这是**镜子，不是闸门**：某一维更靠 0 分端，说明「这里还能更高级」，据此**有针对性地调整**——而不是按总分判定、机械重做视觉层。模型自己判断哪些值得提升、哪些是本页内容/已认可样例的合理选择。
...
18px 为正文基准，向上×1.25（取整）——以下是模型**优先借鉴的默认梯档，不是闭集**：
...
> **为什么用梯档**（让模型「优先」落在上面，而不是禁令）：同一比例族读起来「有设计感」，任意散值（17 / 19 / 24 / 29 …）会破坏隐含比例、显得「随手摆放」。所以**默认借鉴这套阶梯**——但这是**偏好的理由，不是硬性禁止**：当内容或本 deck 已认可的样例（`exemplars/`）更合适时，可以偏离（例如密集数据页用 20px 注释更耐读）；正文基准换成 16 或 24，整条梯档随之平移。导出前的自检只**记下**偏离、据其判断是否仍清晰，**不据此判页失败、不强制重做**。
>
> 同一原则的独立软参考见 #164（`references/type-scale.md`）；本节是「逐节把硬规则软化」的一部分，方向见 #163。
...
<text x="64" y="33" font-size="22" font-weight="700"
...
导出前逐条回看——命中处**多半值得改进**（它们是「廉价/AI 感」的常见来源）；据本页内容与已认可样例判断后调整，而不是一刀切机械重做：
