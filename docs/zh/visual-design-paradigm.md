...
>
> 全文是**范式级参考**：讲「什么算高级、为什么」，模型据此**权衡判断**——当本页内容或该 deck 已认可的样例（`exemplars/`）更合适时自然偏离。它是判断依据，不是逐条硬执行的规则。技术正确性的硬约束（禁用特性 / viewBox / XML）另见 [`shared-standards.md`](../../skills/ppt-master/references/shared-standards.md)。
...
下面十维是判断一页「高级 vs 廉价」的视角：每维左端是廉价/线框图的样子、右端是高级/定制的样子。起草后对照回看，把还偏左的维度往右推。
...
字号循一条比例梯档：18px 正文为基准，向上 ×1.25 取整。同一比例族让层级「成系统」、读起来是设计过的；散落的非梯档值（17 / 19 / 24 / 29 …）会打断这个隐含比例，正是「随手摆放」的廉价感来源。梯档随正文基准平移——基准取 16 或 24，整条跟着变。
...
这些是「廉价/AI 感」的常见来源。起草后逐条回看，命中处通常值得改：
