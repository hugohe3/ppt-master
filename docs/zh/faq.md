...
## Q: 没有 API Key 也能用网络图片吗？

可以。PPT Master 支持从 **Openverse** 和 **Wikimedia Commons** 零配置搜索网络图片——完全免费，无需 API Key。这些来源提供开放授权（CC BY、CC BY-SA、CC0、公共领域）的合法可复用图片。

在策略师阶段，如果没有图片生成 API Key 且演示文稿需要配图，系统会询问是否启用网络图片搜索。授权元数据会自动采集并写入演讲者备注。

可选的密钥供应商（**Pexels**、**Pixabay**）可提供更高质量的商业图片——在 `.env` 中设置 `PEXELS_API_KEY` 或 `PIXABAY_API_KEY` 即可启用。
