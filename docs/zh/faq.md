...
默认会把可合并的正文导出成一个可编辑的 PowerPoint 文本框。SVG 中已经排好的视觉行会在框内保留为硬换行；较大行距和列表项保留为独立段落。只有显式标记 `data-pptx-break="soft"` 的文字才会在导出时拼接视觉行。文本框仍保留 PowerPoint 的正常自动换行，因此删除硬换行或修改文字后，会按文本框宽度重新排版。
