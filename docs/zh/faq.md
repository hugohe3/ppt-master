...
可以。直接告诉 AI 你想做的主题或场景（如"做一个关于宫崎骏的 PPT"、"介绍我们公司新产品"），Generate PPTX 路线会运行 **topic-research 阶段**，补齐规划所需的事实基础与来源记录。已有部分材料时，只补实现用户目标仍缺少的事实；如果用户要求只使用原材料，则不做外部补充。研究网页只导入文字；其图片仅在常规搜图失败后按需尝试一张。
...
```bash
uvx ppt-master svg-to-pptx <project> -t push       # 换转场效果
uvx ppt-master svg-to-pptx <project> -t none       # 关闭转场
uvx ppt-master svg-to-pptx <project> -a auto       # 开启页内元素入场（按 group id 自动映射效果）
uvx ppt-master svg-to-pptx <project> --animation entrance_fade # 开启并改用单一规范效果
uvx ppt-master svg-to-pptx <project> --animation emphasis_spin # 原生强调效果
uvx ppt-master pptx-animations --list             # 完整分类效果清单
uvx ppt-master svg-to-pptx <project> -a auto --animation-trigger on-click   # 单击触发，演讲者控制节奏
```
