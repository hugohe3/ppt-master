...
- **交互式可视化策略师确认页（Step 4）**（[`scripts/confirm_ui/server.py`](../../skills/ppt-master/scripts/confirm_ui/server.py)，字段 schema [`scripts/docs/confirm_ui.md`](../../skills/ppt-master/scripts/docs/confirm_ui.md)） — 策略师确认阶段从纯聊天升级为浏览器可视化页面，Step 4 默认自动拉起：可枚举字段（canvas / mode / visual_style / 条件式模板遵循方式 / 图标 / 图片用法 / 公式与生成策略）从 `catalogs.json` 出常用项、生成型字段摆候选——配色 swatch、字体实时预览（CJK / Latin 各自独立）、AI 图 rendering × palette 候选；并支持自定义 HEX 即时 swatch 反馈、配色 × 字体组合的实时合成预览、canvas 尺寸联动建议正文字号区间。模板遵循方式只在 Step 3 实际加载 Deck/Layout 模板时显示，默认推荐 `adaptive`；`strict` 才进入全页 roster 映射和兼容原生结构保留。与 Step 6 Live Preview **共用 `5050` 端口**（永不同时跑，确认页在 Step 4 末尾自动 `--shutdown` 让出端口；端口被占则自动顺延到下一个空闲口）。原则上**聊天始终是 canonical 通道、确认页只是便利层**：页面写出的 `result.json` 对推荐值具权威性、由下游就地消费（图片计划 / `image_strategy` / 字体 / split 与 refine-spec 开关都从中读取），任何打不开 / 超时 / 无 GUI 的情况都无损回退到聊天总结路径
...
> **已支持边界**：`template-fill-pptx` 直接回填选中的源页面；`create-template` 先把结构完整的源文件收敛成经过确认的 SVG 模板，并可保留原生契约。下游严格套用时才激活 `preserve` 引用精确源版式，适应性使用仍走 baseline。仍不做的是未经审查、没有契约的任意第三方 placeholder 全自动文字替换。
