...
**对称改动两个 server**：`scripts/confirm_ui/server.py`、`scripts/svg_editor/server.py`。
...
| `project_management/cli.py:44,829,838,1002` | import 常量 + 3 处 | import 改为 `from project_management.paths import REPO_ROOT, projects_root, ...` |
...
- Step 3 新增「**fork 修改文件清单**」小节：列出上述 5 文件，策略改为「保留 fork 的 Windows/uvx 适配（token 校验、`PPT_MASTER_PROJECTS`、规范化路径比较），合入上游功能改动」，注明每个文件的关键适配点
...
| auto-tag.yml Gate 2/3（python3/uv run 残留） | 文档修复后无新残留；措辞避免 python3 模式 |
...
| check-uvx-migration.yml | 仅 merge commit 触发，本次普通功能提交 |
...
4. 文档修复：`auto_fix_uvx.py` 运行 → 32 处修复；`rg 'python3.*SKILL_DIR.*scripts'` 零残留
