...
- With edge or MiniMax subtitles, one matching subtitle file per slide under `<project_path>/notes/subtitles/` (`01_cover.srt`, `02_market_landscape.srt`, …). Each file uses a page-local timeline with a `00:00:00,000` origin. Both providers' word timings are regrouped into the same compact cue format.
