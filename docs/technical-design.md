...
**Why native-object reconstruction uses markers, not automatic object replacement.** The standalone `pptx_to_svg.py` importer can emit SVG fallback content plus `data-pptx-native` metadata for supported unmerged tables and conservative classic-chart caches. Table metadata records the source table-style ID and supported direct solid cell fills/basic text formatting, but the SVG fallback currently resolves direct cell properties only; inherited built-in/custom table-style appearance is not guaranteed. The importer also retains chart title/legend/axis titles and plot-level data-label flags for area/bar/column/line charts when the current schema can represent them. Exporter-canonical classic charts recover canonical solid series/slice colors and exact one- or two-paragraph title metadata. The default export still consumes fallback children as regular DrawingML shapes; only `--native-objects` activates editable PowerPoint tables/charts. Unsupported table cases — including merged cells, direct borders, non-solid fills, and mixed rich text — keep the rendered SVG table. Unsupported chart cases — including ChartEx, rotated/flipped/3D/combo charts, unsupported data-label scopes/types, custom axis semantics, trendlines/error bars, unsupported subtype options, sparse or non-finite caches, and date/formatted category axes — keep a baked preview when one exists. Fallback-only objects carry `data-pptx-native-status`; the quality checker and native-object export report them as warnings. A chart without a baked preview instead carries `visual=placeholder` / `route=reconstruction-only`; validation and export warn, default export keeps the placeholder, and `--native-objects` may reconstruct it when a valid active native-chart payload is available. Invalid status declarations remain errors. This importer/exporter pairing is a reconstruction aid, not a preservation-route substitute for `template-fill-pptx` or `native-enhance-pptx`.
