...
**Why native-object reconstruction uses markers, not automatic object replacement.** The standalone `pptx_to_svg.py` importer can emit SVG fallback content plus `data-pptx-native` metadata for verified text-grid tables and conservative classic-chart caches. The table subset now includes unmerged grids, canonical rectangular merge topology with empty slaves, safe solid/no-fill per-side borders, and plain multi-paragraph cells whose run styling is uniform. Noncanonical or overlapping merges, nonblank slaves, unsafe border XML, non-solid fills, and mixed run-level rich text still fail closed. The normalized fallback for table style `{5C22544A-7EE6-4342-B048-85BDC9FD1C3A}` resolves `wholeTbl`, `firstRow`, horizontal banding, theme colors/fonts, and direct overrides; this does not imply a full built-in/custom style registry. For supported parsed column/bar/line/area, pie/doughnut, scatter, and bubble charts, a missing baked preview is replaced by a deterministic readable SVG fallback with `visual=normalized`. The importer additionally covers the verified column/line/area combo subset, canonical four-series OHLC stock charts, and area charts with numeric date axes. Combo plots may retain independent primary/secondary category caches and workbook ranges. Axis preservation is a closed category/value role contract for kind, position, visibility, label position, number format, min/max/major unit, reversal, and major gridlines — not a full `AxisSpec`. Common solid/no-fill/line/marker forms and scheme colors are accepted conservatively; unknown series paint/style XML outside the explicit normalization boundaries still fails closed. Safe stock series style may pass the structural gate, but stock series, `hiLowLines`, and up-down bar local styling can still normalize under the editable-first contract. Native replacement may normalize style details absent from the payload, which remains part of the explicit editable-first warning. The importer also retains chart title/legend/axis titles and supported plot-level data-label flags. The default export consumes fallback children as regular DrawingML shapes; only `--native-objects` activates editable PowerPoint tables/charts. Unsupported chart cases — including ChartEx import, rotated/flipped/3D charts, combo or stock variants outside the verified subset, unsupported date-axis families, unmodeled axis semantics, unsupported data-label scopes/types, trendlines/error bars, unsupported subtype options, and sparse or non-finite caches — keep a baked preview when one exists. Active types outside the current renderer, including `of_pie`, combo, and stock, retain `visual=placeholder` / `route=reconstruction-only` when no source preview exists; native opt-in may still reconstruct their valid payload. Every active imported marker is stamped with `data-pptx-fallback-sha256`: stale visible edits, reachable SVG fragment-definition changes, marker-local reference-target changes, or marker transforms are reported by the mandatory checker; default export keeps the SVG, while native replacement hard-fails. Hashless legacy markers remain compatible with a checker/native-route warning. ChartEx emission consumes valid payload colors in the color-style part, without claiming ChartEx import or full source-style fidelity. Invalid status declarations remain errors. This importer/exporter pairing is a reconstruction aid, not a preservation-route substitute for `template-fill-pptx` or `native-enhance-pptx`.
