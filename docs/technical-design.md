...
**Why imported reusable templates preserve instead of reconstruct.** `pptx_template_import.py` emits a paired `native_structure.json` and `source_template.pptx`. After `create-template` confirms that the source master/layout graph is reusable, `pptx_structure.mode: preserve` uses that package as the base, binds generated pages to exact source layout parts, removes SVG-only previews of inherited layers, and restores source placeholder type/index identities. Existing multiple masters and unused template layouts remain available in PowerPoint's layout picker. Raw PPTX plus one-off generated content still routes to `template-fill-pptx`; preserve mode begins only from a reusable template package.

**Why each SVG references a layout instead of becoming one.** A generated SVG is a complete slide instance: its actual title, body, pictures, tables, and charts must remain selectable in Normal View. `spec_lock.md` therefore separates `pptx_layouts` (the output PowerPoint layout key/name for every page) from `page_layouts` (an optional input SVG design reference). Pages reuse one output key only when their static layout layer and placeholder contract match. The exporter reads `pptx_structure.mode` from the lock when no CLI override is present. Within that hierarchy, ownership follows scope rather than paint type: a direct full-canvas solid rect becomes Master `p:bg` when deck-wide, Layout `p:bg` when page-type-specific, and Slide `p:bg` when explicitly marked for that scope or left unmarked in the background plane. This keeps new slides inheriting the right background without putting content or selectable full-canvas rectangles into layouts.
