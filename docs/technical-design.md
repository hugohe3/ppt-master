...
**One coherent sheet per compatible visual family.** Related spots may share one AI illustration sheet when their cell shapes, detail, quality, and semantic needs are compatible. Decorative lettering is grouped by visual identity rather than an identical effect recipe: compatible marks share a sheet, genuinely different identities use separate sheets, and one family splits only when its element geometry or quality needs conflict. The prompt supplies role, placement context, relative visual weight, and energy; the model owns detailed effect composition inside the confirmed deck identity, with controlled glyph-native expression by default and strong or lettering-plus-illustration expression only after an explicit request or confirmed direction. Every sheet uses an explicit flat pure-red/green/blue chroma key whose active channel does not dominate its elements; the same HEX is passed to `slice_images.py --bg`, whose chroma path removes key spill and recovers partial effect alpha. `--strict-alpha` then rejects any nontransparent cell boundary or edge-crossing effect before writing derived files. Successful transparent slices join `images/`, then `analyze_images.py` refreshes their dimensions.
