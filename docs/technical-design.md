...
**One coherent sheet per compatible treatment family.** Related spots may share one AI illustration sheet when their cell shapes, detail, quality, and semantic needs are compatible. Decorative lettering is grouped by artistic treatment first: compatible marks share a sheet, different treatments use separate sheets, and one treatment splits only when its element geometry or quality needs conflict. `slice_images.py --strict-alpha` rejects incomplete background keying before writing derived files; successful transparent slices join `images/`, then `analyze_images.py` refreshes their dimensions.
