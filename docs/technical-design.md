...
**Template contracts are opt-in; Visualization references and icons are not.** The asymmetry is intentional: Layout locks reusable structure, Style locks direction/method defaults, Brand locks identity, and Deck combines a recurring application with identity and structure. Page-local Chart, Structure, and Table references plus icons are reusable primitives that do not by themselves impose a deck-wide contract. Same `templates/` directory, different ownership.
