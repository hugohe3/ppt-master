...
**One rendering lock, inherited deck anchors, and per-image composition.** Before deciding whether to recommend AI imagery, Stage 2 authors one complete deck-wide rendering candidate inside each of its three coordinated design directions. The source decision is separate: the UI hides rendering controls until AI is selected, then reveals those existing candidates without another recommendation pass. Only confirmed AI usage promotes the selected candidate into the execution contract. Image colors are not a second user decision: Image_Generator starts from the core HEX roles in `spec_lock.md colors`, reads only the selected rendering entry or exact custom bases, then interprets them with the completed Design Spec and each resource's `type` or hero-page composition. Rendering may derive coherent tints, material colors, lighting transitions, and atmospheric hues while preserving the core role meanings; it must not replace the deck identity with an unrelated image-only palette. A repeated derived tone may be promoted into a named lock role.
