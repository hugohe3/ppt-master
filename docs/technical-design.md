...
**Why Create Layout / Create Deck have authored and preservation modes.** `pptx_template_import.py` emits layered Master/Layout/Slide references plus native structure facts. `standard` / `fidelity` inspect the complete source Master/Layout and visual evidence, then author compact or broader useful Slide prototypes and topology from the confirmed reusable behavior. Mirror instead materializes each source Slide and only its reachable Layout/Master chain one-to-one, allowing mechanical context completion required by the explicit structured contract. Unreferenced source structures are not mirror output; authored modes may realize useful ones through new complete Slide prototypes. The original PPTX remains immutable analysis evidence, not a packaged export dependency. Create Brand has no structural replication strategy.
