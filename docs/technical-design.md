...
**Why the default native deck has a baseline master, not inferred templates.** `svg_to_pptx.py` starts from the actual blank slide layout relationship in the base package and defaults to `--pptx-structure baseline`. Baseline promotes only low-risk shared structure: the strict-majority identical slide background (every slide must carry an explicit background, and minority slides keep their own overriding fill), plus a common leading prefix of repeated top-level SVG chrome whose exact id tokens say `logo`, `footer`, `header`, `watermark`, `chrome`, `pageNumber`, or `slideNumber`. Chrome promotion also requires identical generated OOXML, no slide-timing reference, and the same leading z-order across a strict majority of slides sharing the master; overlay chrome stays slide-local. Minority slides — covers and section pages — keep every shape slide-local and are rebound to a generated `Cover` layout with `showMasterSp="0"`, following the PowerPoint convention that page-type differences live in layouts under one master rather than in extra masters, so promoted chrome never leaks onto them. After all pages are generated, an export-only pass assigns `Cover`, `Agenda`, `Section`, `Closing`, or fallback `Content` from exact filename role tokens. It preserves the current master-shape visibility, never compares page visuals or infers placeholders, and moves a family background to its Layout only when every family member carries the exact same explicit `p:bg`; actual page content remains on the Slide. Image relationships for promoted chrome are copied to the slide master, base-template slide layouts no generated slide references are pruned from the package, and `pageNumber`/`slideNumber` chrome whose text exactly equals the slide's display number (honoring `firstSlideNum`) becomes an auto-updating slide-number field so reordering slides in PowerPoint renumbers pages. This gives ordinary decks a useful master/layout roster without constraining SVG authoring or spending model context on layout planning.
...
**Why imported reusable templates retain native structure without forcing it.** `pptx_template_import.py` emits a paired `native_structure.json` and `source_template.pptx`. `create-template` may retain that pair when the source master/layout graph is reusable, while still building the ordinary SVG template roster. A later Strategist confirmation chooses template adherence: `strict` may set `pptx_structure.mode: preserve`, use the source package as the base, bind every generated page to an exact source layout, remove SVG-only previews of inherited layers, and restore source placeholder type/index identities; `adaptive` keeps template SVGs as optional references and exports through baseline. Existing multiple masters and unused template layouts remain available in PowerPoint's layout picker only on the strict preserve path. Raw PPTX plus one-off generated content still routes to `template-fill-pptx`; preserve mode begins only from a reusable template package.

**Why each SVG references a layout instead of becoming one.** A generated SVG is a complete slide instance: its actual title, body, pictures, tables, and charts must remain selectable in Normal View. `spec_lock.md` therefore separates `pptx_layouts` (the output PowerPoint layout key/name for every structured page) from `page_layouts` (an input SVG design reference). Confirmed `template_adherence: strict` requires a template SVG for every page and may activate preserved native layouts; `adaptive` permits missing `page_layouts` rows and keeps native export on baseline. Pages reuse one output key only when their static layout layer and placeholder contract match. The exporter reads `pptx_structure.mode` from the lock when no CLI override is present. Within that hierarchy, ownership follows scope rather than paint type: a direct full-canvas solid rect becomes Master `p:bg` when deck-wide, Layout `p:bg` when page-type-specific, and Slide `p:bg` when explicitly marked for that scope or left unmarked in the background plane. This keeps new slides inheriting the right background without putting content or selectable full-canvas rectangles into layouts.
