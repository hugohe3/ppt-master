...
**Why template-mode Master text styles are deliberately narrow.** Template export requires the locked `typography.title` and `typography.body` sizes, then writes the title size to every Master `p:titleStyle` level and the body size to every `p:bodyStyle` / `p:otherStyle` level. The patch changes only `p:txStyles//a:defRPr@sz`; direct run sizes on generated slides remain unchanged, so the authored SVG still controls initial rendering. Missing title/body locks fail template export, while `baseline`, `preserve`, and `flat` retain their previous behavior.
