...
```bash
uvx ppt-master svg-to-pptx <project> -t push       # different transition
uvx ppt-master svg-to-pptx <project> -t none       # disable transitions
uvx ppt-master svg-to-pptx <project> -a auto       # enable per-element entrance (effect mapped from group id)
uvx ppt-master svg-to-pptx <project> --animation fade        # enable with a single effect
uvx ppt-master svg-to-pptx <project> -a auto --animation-trigger on-click   # presenter-paced reveals
```
...
Large imported SVGs may contain native-shape metadata, hidden carriers, and preview fingerprints. That lossless representation stays in the temporary analysis workspace, while the model reads a lightweight inspection projection. The projection is never an export source. `standard` / `fidelity` author project-canonical SVG and use compact authored-preset groups only for exact registered preset matches. Mirror materializes from the lossless source, reuses only metadata already supported by the converter on unchanged Slide-local/slot objects, and keeps an SVG fallback for unsupported or edited objects.
