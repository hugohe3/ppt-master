...
- `/generate-outline`：适合先做内容规划、审大纲，并把图片素材准备完成
- `/render-from-outline`：适合已有大纲和素材后继续出 SVG / PPT
- `/generate-ppt`：适合需要串联完整流程时使用
...
| **1.2. 大纲校验**     | 工具 + 人工审阅 | 校验页面执行卡、确认大纲可独立交付        | 已确认大纲             |
...
7. 图片使用：
   ...
   ⚠️ 若选择包含「B) 用户提供」，八项确认后需运行 `python3 tools/analyze_images.py <项目路径>/images`，
