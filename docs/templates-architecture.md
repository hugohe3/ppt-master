...
Each index maps one-to-one with its physical directory; fields are trimmed to what Strategist actually needs to pick, following the compact "meta + summary" pattern used by the Visualization family indexes ([Chart](../skills/ppt-master/templates/charts/charts_index.json), [Structure](../skills/ppt-master/templates/structures/structures_index.json), and [Table](../skills/ppt-master/templates/tables/tables_index.json)) while preserving structured metadata that helps selection.
