...
| Picture shadow or glow | One registered effect filter on `<image>`; imported crops use the private outer carrier | Native `p:pic/p:spPr/a:effectLst` | `Approximate`; one effect round-trips | Other primitives, effect DAGs, and multiple independent effects remain unsupported |
