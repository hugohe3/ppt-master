...
| Imported text-run effect | Unchanged `metadata[data-pptx-part="txbody"]` on a logical shape; import-only blocking effect status for separate vertical-text output | Original native run effect inside `p:txBody` | `Native-stable` only while the raw payload remains usable; edits or vertical-text normalization that would drop a non-empty run `effectLst` / `effectDag` block | Not public authoring syntax; relationship-bearing text bodies and table-cell text bodies remain outside this guard |
