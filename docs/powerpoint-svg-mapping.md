...
| Picture shadow or glow | One registered effect filter directly on an unclipped `<image>`; a clipped image or imported crop uses its exact single-picture outer carrier | Native `p:pic/p:spPr/a:effectLst` | `Approximate`; one effect round-trips as one editable picture | `filter` and `clip-path` cannot share one `<image>`; Layer/Placeholder/Binding/replacement ownership stays outside the carrier; ordinary group filters, other primitives, effect DAGs, and multiple independent effects remain unsupported |
