...
| Authored multiline and paragraph text | One `<text>` with positioned direct `<tspan>` lines | By default, one no-wrap text frame with authored line breaks retained; semantic paragraph boundaries remain `a:p` | `Native-normalized` | `--reflow-text` permits PowerPoint reflow; `--no-merge` emits one shape per visual line |
