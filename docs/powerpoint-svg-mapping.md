...
| Superscript and subscript in ordinary text | No registered generated-SVG control; `baseline-shift` is invalid. Literal Unicode superscript/subscript glyphs and formula markers are separate authoring choices | The main generation route does not author `a:rPr@baseline` on ordinary runs | `Direct preservation` applies where a source-preserving native workflow leaves the owning text body intact; the SVG round-trip is `Native-stable` only through unchanged imported `txBody` metadata, and PPTX-to-SVG does not reconstruct the baseline shift in visible SVG | Structured mathematics uses editable OMML. A Unicode glyph remains literal text and does not become a native baseline shift |
