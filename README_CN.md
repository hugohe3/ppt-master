...
**A) AI 生图** — Agent host 提供原生生图工具时可直接使用；也可通过 `image_gen.py` 配置 `IMAGE_BACKEND` 和供应商 `*_API_KEY`。host-native 生图不需要另配供应商生图 API Key，直接要求 Agent 使用自身生图工具即可。`uvx ppt-master image-gen --list-backends` 查看供应商后端清单。`gpt-image-2` 目前综合质量最佳。
