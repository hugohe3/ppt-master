...
class BatchValidator:
    ...
    def validate_project(self, project_path: str) -> Dict:
        ...
        if not info['has_preview']:
            self.summary['missing_preview'] += 1
        if svg_warnings:
        ...
        print(f"   格式: {info['format_name']} | SVG: {info['svg_count']} 个 | 日期: {info['date_formatted']}")
    ...
    def print_summary(self):
        ...
        print(f"  ✅ 完全合格: {self.summary['valid']} ({self._percentage(self.summary['valid'])}%)")
        print(f"  ⚠️  有警告: {self.summary['has_warnings']} ({self._percentage(self.summary['has_warnings'])}%)")
        print(f"  ❌ 有错误: {self.summary['has_errors']} ({self._percentage(self.summary['has_errors'])}%)")
        
        print(f"\n常见问题:")
        print(f"  缺少 README.md: {self.summary['missing_readme']} 个项目")
        print(f"  缺少设计规范: {self.summary['missing_spec']} 个项目")
        print(f"  缺少预览文件: {self.summary['missing_preview']} 个项目")
        ...
        if self.summary['has_errors'] > 0 or self.summary['has_warnings'] > 0:
            ...
            if self.summary['missing_readme'] > 0:
                ...
                print(f"     参考: examples/google_annual_report_ppt169_20251116/README.md")
            
            if self.summary['missing_preview'] > 0:
                print(f"  2. 生成预览文件:")
                print(f"     python3 tools/generate_preview.py --all examples")
            
            if self.summary['svg_issues'] > 0:
                print(f"  3. 检查并修复 SVG viewBox 设置")
                print(f"     确保与画布格式一致")
    ...
    def export_report(self, output_file: str = 'validation_report.txt'):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            for result in self.results:
                ...
                f.write(f"格式: {result['format']} | SVG: {result['svg_count']} 个\n")
