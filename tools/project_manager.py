...
class ProjectManager:
    ...
    def init_project(self, project_name: str, canvas_format: str = 'ppt169',
        ...
        # 获取画布格式信息
        canvas_info = self.CANVAS_FORMATS.get(canvas_format, self.CANVAS_FORMATS['ppt169'])
...
```bash
python3 tools/generate_preview.py {project_dir_name}
```
...
"""
    ...
    def validate_project(self, project_path: str) -> Tuple[bool, List[str], List[str]]:
            ...
            if len(svg_files) == 0:
                ...
                for svg_file in svg_files:
                    if not re.match(r'^slide_\d+_\w+\.svg$', svg_file.name):
                        warnings.append(f"SVG 文件命名不规范: {svg_file.name} (建议: slide_XX_name.svg)")
        ...
        if not re.match(r'^.+_(ppt169|ppt43|wechat|xiaohongshu|story|moments|banner|a4)_\d{8}$', dir_name, re.IGNORECASE):
            warnings.append(f"项目目录命名不规范: {dir_name} (建议: name_format_YYYYMMDD)")
        
        # 检查预览文件
        if not (project_path / 'preview.html').exists():
            warnings.append("缺少 preview.html（可使用 generate_preview.py 生成）")
    ...
    def get_project_info(self, project_path: str) -> Dict:
        ...
        # 检查预览文件
        info['has_preview'] = (project_path / 'preview.html').exists()
...
def main():
    ...
    if command == 'init':
        if len(sys.argv) < 3:
            print("错误: 需要提供项目名称")
            print("用法: python3 tools/project_manager.py init <project_name> [--format ppt169]")
            sys.exit(1)
        ...
        try:
            ...
            print(f"3. 运行: python3 tools/generate_preview.py {project_path}")
        except Exception as e:
            print(f"❌ 创建失败: {e}")
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print("错误: 需要提供项目路径")
            print("用法: python3 tools/project_manager.py preview <project_path>")
            sys.exit(1)
        ...
        # 导入预览生成器
        try:
            from generate_preview import PreviewGenerator
            generator = PreviewGenerator()
            output_path = generator.generate(project_path)
            print(f"✅ 预览文件已生成: {output_path}")
        except ImportError:
            print("❌ 无法导入 generate_preview 模块")
            print("请确保 generate_preview.py 在同一目录下")
            sys.exit(1)
        except Exception as e:
            print(f"❌ 生成失败: {e}")
            sys.exit(1)
        ...
        print(f"预览文件: {'存在' if info['has_preview'] else '缺失'}")
