...
class SVGQualityChecker:
    ...
    def _check_forbidden_elements(self, content: str, result: Dict):
        """检查禁用元素"""
        # 检查 foreignObject
        if '<foreignObject' in content or '<foreignobject' in content.lower():
            result['errors'].append("检测到禁用的 <foreignObject> 元素")
        ...
        if '<script' in content.lower():
            result['warnings'].append("检测到 <script> 元素（可能影响安全性）")
    ...
    def _check_fonts(self, content: str, result: Dict):
        ...
        if font_matches:
            ...
            # 检查是否使用了系统 UI 字体栈
            recommended_fonts = ['system-ui', '-apple-system', 'BlinkMacSystemFont', 'Segoe UI']
            
            for font_family in font_matches:
                has_recommended = any(rec in font_family for rec in recommended_fonts)
