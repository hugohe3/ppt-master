...
def generate_examples_index(examples_dir: str = 'examples') -> str:
    ...
    for fmt_key in format_order:
        ...
        for info in projects_list:
            ...
            preview_link = f"./{info['dir_name']}/preview.html"
    ...
    content.append("- `preview.html` - 预览页面（推荐）")
    content.append("- `svg_output/` - SVG 输出文件\n")
    
    content.append("**方法 1: 使用预览页面（推荐）**\n")
    content.append("```bash")
    content.append("open examples/<project_name>/preview.html")
    content.append("```\n")
    
    content.append("**方法 2: 使用 HTTP 服务器**\n")
    content.append("```bash")
    content.append("python3 -m http.server --directory examples/<project_name>/svg_output 8000")
    content.append("# 访问 http://localhost:8000")
    content.append("```\n")
    
    content.append("**方法 3: 直接打开 SVG**\n")
    content.append("```bash")
    content.append("open examples/<project_name>/svg_output/slide_01_cover.svg")
    content.append("```\n")
    ...
    content.append("```bash")
    content.append("python3 tools/project_manager.py init my_project --format ppt169")
    content.append("```\n")
    
    # 贡献指南
    content.append("## 🤝 贡献示例项目\n")
    ...
    content.append("2. 生成预览文件: `python3 tools/generate_preview.py examples/<project>`")
    content.append("3. 验证项目: `python3 tools/project_manager.py validate examples/<project>`")
    content.append("4. 更新索引: `python3 tools/generate_examples_index.py`")
    content.append("5. 提交 Pull Request\n")
    ...
    # 页脚
    content.append("---\n")
    content.append(f"*自动生成于 {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} by PPT Master*")
