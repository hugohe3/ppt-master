...
- Topic-only or fact-insufficient inputs run [`topic-research`](skills/ppt-master/workflows/stages/topic-research.md) inside the selected Generate profile's source intake; retained webpages are imported as source packages whose embedded images remain source-extracted inventory, while independent image acquisition stays in later resource preparation.
