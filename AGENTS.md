...
>
> 执行 `/generate-outline`、`/render-from-outline` 或 `/generate-ppt` 前必须阅读本文件。
...
| 部分 | 内容 |
|------|------|
| [工作流程](#工作流程) | 总纲级流程、阶段边界与 workflow 入口 |
...
> 📌 **本节只保留总纲级流程与阶段边界**。具体执行步骤请进入对应 workflow。

## 工作流总览

```
源文档 → 创建项目 → 模板选项 → 第一阶段（大纲与素材准备）→ 第二阶段（渲染与导出）
```
...
- 负责范围：源材料转换、项目初始化、模板选项确认、Strategist、大纲校验、Image_Generator（如需要）
- 结束标准：`设计规范与内容大纲.md` 已确认，`images/` 中所需素材已就绪
- 对应 workflow：`/.agent/workflows/generate-outline.md`

### B. `/render-from-outline`

- 负责范围：Executor、讲稿生成、后处理、导出 PPTX
- 前置条件：大纲已确认，所需图片素材已准备完成
- 对应 workflow：`/.agent/workflows/render-from-outline.md`

### C. `/generate-ppt`

- 负责范围：串联 A + B
- 适用场景：需要从源材料一路生成到 PPT 的完整流程
- 对应 workflow：`/.agent/workflows/generate-ppt.md`

> ⚠️ `AGENTS.md` 只定义总纲级流程和全局规则，不重复展开各 workflow 的细节步骤。

## 强制前置检查

在执行任一 workflow 之前，必须先确认：
...
- 必须先创建项目并归档 `sources/`
- 必须先确认模板选项，再进入 Strategist
- Strategist 必须完成八项确认并生成《设计规范与内容大纲》
- 必须运行 `python3 tools/outline_quality_checker.py <项目路径>`
- 若图片方式包含 AI 生成，必须在本阶段完成 Image_Generator
- 本阶段结束时，不要求存在 `svg_output/`

### 第二阶段：渲染与导出

- 输入必须是已确认的大纲和已就绪素材
- Executor 必须以《设计规范与内容大纲》为主执行，不得回退为全量扫描 `sources/`
- Executor 必须完成两部分产物：`svg_output/` 与 `notes/total.md`
- 后处理顺序固定：`total_md_split.py` → `finalize_svg.py` → `svg_to_pptx.py`
...
| 大纲未确认就进入渲染 | 先完成 `outline_quality_checker.py` 和大纲审阅 |
| 图片尚未就绪就进入 Executor | 先在第一阶段完成 Image_Generator 或图片归档 |
| 直接从 `svg_output` 导出 | 使用 `-s final` 从 `svg_final` 导出 |
| 忘记拆分备注或后处理 | 按固定顺序执行 `total_md_split.py`、`finalize_svg.py`、`svg_to_pptx.py` |
...
- **创建项目后立即询问模板选项**（A 使用已有 / B 不使用）— 这是全局阶段顺序，不是策略师职责
...
- **图片生成顺序**：如果图片方式**包含**「C) AI 生成」（如 C、B+C、C+D），**必须**在第一阶段完成 Image_Generator，再进入 Executor 阶段
- **Executor 两阶段**：SVG 页面生成（视觉构建）完成后，**必须**进入逻辑构建阶段生成演讲备注 `notes/total.md`，**禁止**跳过此步骤直接进入后处理
- **Executor 逐页生成规则**：执行师生成 `svg_output/` 时，**必须一页一页生成 SVG**，**禁止**使用 Python 脚本、批处理代码或程序化模板循环一次性批量生成整套页面；正式质量检查必须在**全部 SVG 页面生成完成后**统一执行
