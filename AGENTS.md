...
- ⚠️ **Strict serial Step execution**: Every Step in the pipeline MUST be executed sequentially — bundling, batching, or parallelizing Steps is FORBIDDEN
- ⚠️ **build_slides.py authoring must stay in the main agent**: During Executor Step 6, the project's `build_slides.py` MUST be authored/edited by the current main agent with full project context. Delegating to sub-agents is FORBIDDEN
- ⚠️ **Two-phase build with 5-page style gate**: Executor Step 6 follows phase A (author 5-page sample, run, get user style approval) → phase B (append remaining pages in 5-page batches, validating after each batch). See `skills/ppt-master/references/build-slides-spec.md`
