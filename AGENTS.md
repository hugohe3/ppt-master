...
源内容转换工具（如需要）
          ...
          ▼
【创建项目文件夹】← ← ← ← ← 获取原始材料后立即执行
  python3 tools/project_manager.py init <名称> --format <格式>
          │
          ▼
┌─────────────────────────────────────────┐
│ 询问模板选项（三选一）                    │
...
【导出 PPTX】← ← ← ← ← 默认由 AI 自动执行
  python3 tools/svg_to_pptx.py
          │
          ▼
Optimizer_CRAP (可选优化，**在完整初版后**不满意时才考虑)
          │
          ▼
如执行优化：重新运行【后处理】与【导出 PPTX】
```
...
> - 后处理和导出步骤以 **AI 自动执行为主**（需要时可手动运行命令）
> - Optimizer_CRAP 仅在质量不足时使用；**须在完整初版产出后再考虑，优化后需重新运行后处理与导出**
