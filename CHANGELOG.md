...
- **项目管理工具** - 完整的项目生命周期管理
  - 新增 `tools/project_manager.py` 项目管理工具
    - `init`: 初始化新项目（自动创建标准目录结构和模板文件）
    - `validate`: 验证项目完整性（检查必需文件、SVG 命名、viewBox 等）
    - `info`: 查看项目信息（统计 SVG 数量、检测格式等）
    - `preview`: 快捷生成预览文件
  - 支持 8 种画布格式：ppt169, ppt43, wechat, xiaohongshu, moments, story, banner, a4

- **项目结构规范文档** - 标准化项目组织
  - 新增 `docs/project_structure.md` 详细规范文档
  - 定义标准项目结构和命名规范
  - 项目命名格式：`{项目名称}_{format}_{YYYYMMDD}`
  - SVG 文件命名格式：`slide_{序号}_{名称}.svg`
  - 提供完整的项目生命周期指南
  - 包含迁移指南和最佳实践
...
- **工具文档重构**
  - 完全重写 `tools/README.md`，新增项目管理和预览生成工具说明
  - 添加工作流集成示例和故障排除指南
  - 更新所有工具的使用说明和示例

- **主文档更新**
  - README.md: 新增工具集章节，更新项目创建流程
  - AGENTS.md: 新增项目管理和预览生成命令说明
  - 统一预览方法说明（推荐使用 preview.html）
