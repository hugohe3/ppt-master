...
**策略师主动建议**: 根据已确认的风格和场景，直接给出图标使用建议，而非仅提问。
...
- **轻松社交场景**: "考虑到这是小红书分享内容，我建议选择 **A) Emoji 表情**，更加亲和活泼，契合平台调性。"
- **专业商务场景**: "基于高端咨询风格的选择，我建议使用 **C) 内置图标库**，风格统一且专业。图标库包含 640+ 个图标，涵盖箭头、数据图表、商业、状态等分类，详见 `templates/icons/README.md`。"
- **品牌定制场景**: "如果您有公司的品牌图标或指定素材，建议选择 **D) 自定义图标**，请提供图标文件夹路径。"
- **特殊定制需求**: "如果需要与设计风格高度匹配的独特图标，可选择 **B) AI 生成**，将根据配色方案和视觉风格绘制简洁的矢量图标。"
...
> **流程说明**: 无论选择哪种方式，SVG 生成过程都不会阻塞。选择 B（AI 生成）时，执行器将直接用 SVG 基本图形绘制简洁图标。

**g. 图片使用确认**:

**策略师主动建议**: 根据内容类型和使用场景，直接给出图片使用建议。

| 选项 | 方式 | 说明 | 适用场景 |
|------|------|------|----------|
| **A** | **不使用图片** | 纯文字+图表+图标，无需额外图片 | 数据报告、流程说明、纯信息展示 |
| **B** | **用户提供图片** | 使用用户指定的图片文件 | 有品牌素材、产品图、实景照片 |
| **C** | **AI 生成图片** | 先用占位符预留位置，后续由 AI 生成 | 需要插图但无现成素材 |
| **D** | **占位符预留** | 用占位框标注图片位置，后续替换 | 图片待定、需设计师后期配图 |

**建议示例**:

- **数据分析报告**: "本次内容以数据图表为主，我建议选择 **A) 不使用图片**，保持专业简洁的咨询风格。"
- **产品介绍/营销**: "建议选择 **B) 用户提供图片**，使用产品实拍图或品牌素材提升说服力。"
- **概念解说/教程**: "如果没有现成配图，我建议选择 **C) AI 生成图片**，先完成 SVG 生成，后续再补充 AI 配图。"
...
**选择 C/D 的占位符格式**:
占位符将使用统一的视觉样式，包含图片描述供后续生成/替换：
```xml
<rect x="..." y="..." width="..." height="..." 
      fill="#F5F5F5" stroke="#CCCCCC" stroke-width="2" 
      stroke-dasharray="8,4" rx="8"/>
<text ...>[图片：具体说明，如"展示团队协作的场景图"]</text>
```

> **流程说明**: 无论选择哪种方式，SVG 生成过程都不会阻塞。
> - 选择 B（用户提供）：使用外部链接引用图片，生成后可用工具转换为 Base64
> - 选择 C（AI 生成）：先用占位符预留位置和描述，SVG 生成完成后再统一生成图片
> - 选择 D（占位符）：标注位置和说明，后续手动替换
