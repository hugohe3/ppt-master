...
完成设计规范输出后，**先询问模板选项**，再根据图片使用选择提示下一步。
...
**模板选项**（三选一）:
模板可以确保页面风格统一，请选择您的模板策略：

A) **使用已有模板** - 使用现有的模板文件
   └→ 适合：已有模板资源、快速启动
   └→ 来源：用户自备模板 或 参考 templates/layouts/ 示例库

B) **生成新模板** - 由 Template_Designer 根据设计规范全新生成
   └→ 适合：需要定制化风格、品牌个性化设计
   └→ 将生成封面、目录页、章节页、内容页、结束页等模板

C) **不使用模板** - Executor 根据设计规范自由生成每一页
   └→ 适合：页面较少、布局差异大、简单快速生成
...
```
**下一步**: 将已有模板复制到项目 `templates/` 目录。

模板来源：
- 用户自备模板（从其他项目或外部来源获取）
- 参考示例库：templates/layouts/[general|consultant|consultant_top]/

操作：
1. 将模板文件复制到项目 `templates/` 目录
2. 根据【设计规范】调整模板配色和文字
3. [后续流程...]
```
...
```
**下一步**: 请调用 **Template_Designer** 角色生成页面模板。
...
Strategist (本角色)
    ...
    ├─ 模板选项（三选一）
    │       │
    │       A) 使用已有模板 → 从 templates/layouts/ 复制 → 项目 templates/
    │       │
    │       B) 生成新模板 → Template_Designer → 项目 templates/
    │       │
    │       C) 不使用模板 → 直接进入下一阶段
    │                                               │
    ├───────────────────────────────────────────────┤
