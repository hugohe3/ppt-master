...
**策略师询问用户是否需要为每页 PPT 添加讲稿（Speaker Notes）**：

| 选项 | 方式 | 适用场景 |
|------|------|----------|
| **A** | 不需要 | 仅展示用途，无需演讲 |
| **B** | 需要（用户提供） | 有现成讲稿或用户自行编写 |
| **C** | 需要（AI 生成） | 需要 AI 根据内容大纲生成讲稿 |

**选择 B/C 时**，在规范中添加「讲稿说明」：
...
> ⚠️ **流程衔接**: 如果用户选择 **B) 用户提供** 或 **C) AI 生成**，在 PPTX 导出后，需执行讲稿添加流程。使用 `tools/add_speaker_notes.py` 工具添加讲稿。
