...
你输出的《设计规范与内容大纲》必须是 **可执行蓝图**，而不是松散的规划摘要。

**保留原则**：

- 必须保留原有大纲的 12 章结构与核心信息
- 你做的是在原大纲基础上补充执行细节，而不是删减原有内容
- 所有执行所需信息都应被系统化写入对应章节，例如项目执行要求、页面执行卡、执行就绪检查

**标准定义**：

- 进入 Executor 阶段后，执行师应以《设计规范与内容大纲》作为主执行依据与主事实锚点
- 如需补充事实、核验数字或提取原文，Executor 只能查阅大纲中已标记或已选定的 `sources/`
- 聊天窗口可作为补充输入，但凡影响结构、关键结论或关键事实的内容，应尽量先回写到大纲
- 如果某个事实、数字、案例、判断会影响页面内容，它必须先被写入大纲

**完成标准**：

- 即便不主动全量扫描 `sources/`，Executor 仍能以大纲为主稳定完成页面生成
- 每页都必须写成“页面执行卡”，不能只写页面标题和几个 bullet
- 所有关键数字、结论、来源编号、页面目标、推荐布局、讲稿要点都必须进入大纲
...
> ⚠️ **流程衔接**: 输出设计规范后，应先运行 `outline_quality_checker.py` 并完成大纲审阅。若用户选择 **C) AI 生成**，则在大纲确认后提示调用 **Image_Generator** 角色完成图片生成；图片归集到 `images/` 目录后，再进入 Executor 阶段。
...
输出的《设计规范与内容大纲》**必须保留以下 12 个章节**，且章节名称应与 `templates/design_spec_reference.md` 保持一致；你需要在这些既有章节下补充执行信息，而不是改名替换：

| 章节                   | 内容要求                                                  |
| ---------------------- | --------------------------------------------------------- |
| **一、项目信息** | 保留原字段，并补充核心目标、总体叙事、表达禁区 |
| **二、画布规范** | 保留原字段，并补充标题区/正文区/页脚区说明 |
| **三、视觉主题** | 保留原字段，并补充图标/图片/风格执行策略 |
| **四、排版体系** | 保留原字段，并补充密度约束 |
| **五、布局原则** | 保留原字段，并补充推荐优先布局与禁用布局 |
| **六、图标使用规范** | 保留原字段，并补充图标名称清单、调用方式与使用策略 |
| **七、图片资源清单** | 保留原字段，并补充资源使用约束 |
| **八、内容大纲** | 保留原页面结构，并补充来源摘要、页面映射与“页面执行卡” |
| **九、演讲备注要求** | 保留原字段，并补充时长、风格、页级备注约束 |
| **十、技术约束提醒** | 保留原规则，并明确大纲优先、sources 仅作按需查证 |
| **十一、设计检查清单** | 保留原检查项，并补充执行就绪检查 |
| **十二、下一步** | 保留原流程，并加入 `outline_quality_checker.py` |
...
> 4. 运行 `python3 tools/outline_quality_checker.py <项目路径>`，未通过前不得进入 Executor
