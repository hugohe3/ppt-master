...
> ⚠️ **这是一个可选角色**：用户可以跳过模板阶段，直接进入 Executor 生成内容。
...
> **工作流位置**: Strategist → **Template_Designer (可选)** → (Image_Generator) → Executor
...
模板文件保存到项目的 `templates/` 目录：

```
projects/<项目名>_<格式>_<日期>/
├── templates/           # 项目模板目录
│   ├── 01_cover.svg
│   ├── 02_chapter.svg
│   ├── 03_content.svg
...
如果 `templates/layouts/` 中存在适合的预置模板，可以：

1. **复制预置模板**: 将预置模板复制到项目 `templates/` 目录
2. **调整配色**: 根据项目设计规范修改颜色
