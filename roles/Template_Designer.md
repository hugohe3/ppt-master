...
| 模式 | 触发方式 | 输出位置 |
|------|----------|----------|
| **独立使用** | `/create-template` 或直接描述需求 | `templates/layouts/<名称>/` |
| **流程中使用** | `generate-ppt` 选 C | `projects/<项目>/templates/` |

> ⏭️ **流程中下一步**：Image_Generator（如需 AI 图片）或 Executor
...
> 说明：本角色用于**项目内模板生成**，输出到项目目录的 `templates/`。如需创建**标准化模板库**，请使用 `/.agent/workflows/create-template.md`，输出到 `templates/layouts/`。
