...
- **分阶段逐页生成**(强制):
  1. **视觉构建阶段**: 必须按页面执行卡逐页生成 SVG，直到全部页面完成；正式质量检查放在整套 SVG 生成完成后统一执行，不在中途逐页运行质量检查
  2. **禁止事项**: 不得使用 Python 脚本、代码循环、模板批处理或程序化批量方式一次性生成多页 SVG；逐页人工构建是质量要求，不是可选偏好
  3. **逻辑构建阶段**: SVG 全部定稿并完成统一质量检查后，再统一生成 `notes/total.md`，确保叙事逻辑连贯（Narrative Continuity）
- **技术规范**:
  - viewBox 必须与画布尺寸一致
  - 使用 `<tspan>` 手动换行
