...
- 你以《设计规范与内容大纲》作为主执行依据与主事实锚点。
- 如需补充事实、核验数据或提取原文，只能查阅大纲中已标记或已选定的 `sources/`，不得无边界扩展材料范围。
- 聊天窗口中的补充信息，如会影响页面结构、关键结论或关键事实，应先回写到《设计规范与内容大纲》。
- 如果大纲缺少页面目标、页面结论、必须信息、来源编号、推荐布局、文案结构、备注要点，你必须停止执行并要求先补齐大纲。
- 你不是二次策略师；不自行扩写关键事实，不自行新增结论，不自行补来源。
...
> ⚠️ **额外强制步骤**: 在开始执行前，必须确认大纲已经通过 `python3 tools/outline_quality_checker.py <项目路径>` 检查，且页面执行卡完整。
...
- **以页面执行卡为主执行**: 每页以“页面目标 / 页面结论 / 必须使用的信息 / 来源编号 / 推荐布局 / 文案结构 / 备注要点”为准；如需查证，只回查该页对应来源
- **遵循模板结构**: 如有模板，继承模板的视觉框架
- **分阶段逐页生成**(强制):
  1. **视觉构建阶段**: 必须按页面执行卡逐页生成 SVG，直到全部页面完成；正式质量检查必须在整套 SVG 完成后统一执行，不在中途逐页运行质量检查
  2. **禁止事项**: 不得使用 Python 脚本、代码循环、模板批处理或程序化批量方式一次性生成多页 SVG；顶级咨询风格页面必须逐页推敲
  3. **逻辑构建阶段**: SVG 全部定稿并完成统一质量检查后，再统一生成演讲备注，确保叙事逻辑连贯（Narrative Continuity）
 - **技术规范**:
   - viewBox 必须与画布尺寸一致
   - 使用 `<tspan>` 手动换行
...
> ⚠️ 此处检查时机固定为：**全部 SVG 页面生成完成后**统一执行，不在中途逐页运行正式质量检查。
...
在**所有 SVG 页面生成完成、并完成统一质量检查后**，进入"逻辑构建阶段"，生成完整的演讲备注文稿，在 `notes/total.md` 文件中。

**为什么备注可以统一生成？**
- 备注统一撰写更容易规划转场语（Transition），确保演讲逻辑通顺；但这条规则只适用于备注，不适用于 SVG 页面生成
...
> ⚠️ 本清单用于**全部 SVG 页面生成完成后**的统一质量检查，不在中途逐页执行正式质检。
