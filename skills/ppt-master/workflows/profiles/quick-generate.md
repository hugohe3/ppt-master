...
**Typography**: name a concrete target-installed/approved family under `shared-standards-core.md` §4.1, never a lock or the host's fonts. Before P01 measure one representative line per recurring role with `python3 ${SKILL_DIR}/scripts/text_measure.py measure` (batched through `--stdin`) and keep "≈ N chars per W px" in context (the checker adds wrapping headroom, so characters × font size underestimates by roughly 15%); every later page sizes zones from that per-font arithmetic — write the sentence first, fit the zone to it, and never trim wording to satisfy an estimate; measure again only for a role or size never calibrated, and `wrap` only a genuinely long paragraph.
