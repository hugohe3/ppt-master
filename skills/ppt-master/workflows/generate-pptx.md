...
> **Default — one sheet per compatible AI visual family (may override for different cell shape, detail, quality, or semantics)**: for lettering, group exact strings by a coherent visual identity rather than an identical effect recipe; compatible marks share a sheet, genuinely different identities use separate sheets, and one family splits only when its element needs conflict. Apply the same family test to spots. Give the model role/context/relative visual weight/energy and apply the controlled-default/high-expression boundary in [image-generator.md](../references/image-generator.md) §5.3. A lettering sheet contains no scene or page chrome; keep it unplaced and place/project only successful transparent `slice` rows. Contract: [image-generator.md](../references/image-generator.md) §4.3.
