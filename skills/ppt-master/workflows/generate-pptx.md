...
> ⚠️ **Main-agent only**: SVG generation stays in the current main agent — page design depends on full upstream context. Cadence: P01 → first-page gate → remaining pages (one page gate per first-exercised item) → final gate, in one context, no batches or other mid-run checker calls; reload under Context validity above after context invalidation.
