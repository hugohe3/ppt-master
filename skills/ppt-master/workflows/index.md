...
| `distill-layouts` | [`distill-layouts.md`](./distill-layouts.md) | User asks to promote finished baseline pages, or a template project reaches deferred structure finalization | Completed SVG pages; template route also requires complete `page_layouts` | Not for preserve, legacy immediate-template, or cross-project registration | After Step 6 or as a post-export re-entry | Complete all-page `pptx_layouts` plus SVG structure metadata | Baseline blocks for missing page selection; template blocks for missing/invalid prototypes |
