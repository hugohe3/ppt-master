...
| `create-template` | Each reusable template SVG is a complete visual reference plus explicit PowerPoint structure metadata. Library and project outputs use the same complete workspace shape: `templates/`, `images/`, `icons/`, and `exports/<id>_template_preview.pptx`. |
...
| Explicit template workspace path | User provides a current layout/deck workspace root containing `templates/design_spec.md`, a compatible legacy-flat layout/deck root containing `design_spec.md`, or an existing brand directory under its unchanged direct-spec contract | Main `SKILL.md` Step 3 | Fuzzy template lookup by bare name; passing a current workspace's inner `templates/` directory instead of its root | Path resolves, frontmatter kind is valid, and every deck/layout SVG satisfies the current structured Master/Layout/slot contract | Stage `templates/`, `images/`, and `icons/` from the explicit workspace root into the target project peers; consume the target project's own root in place; never consume the review-only `exports/` directory | Route only a structurally legacy SVG contract to [`restore-pptx-structure`](./restore-pptx-structure.md) before Step 3; a legacy-flat directory shape alone remains compatible and does not trigger restoration |
...
| Restore legacy PowerPoint structure | Existing project/template SVGs use unmapped `baseline`, `preserve`, `layout_strategy: distill`, `data-pptx-layout-kind`, `distilled`/`utility`, direct atomic placeholders, or lack root Master identity | [`restore-pptx-structure`](./restore-pptx-structure.md) before generation/export or before Step 3 template consumption | Legacy SVG project/package exists; original PPTX/native facts are used when available; a flat package directory by itself is not a trigger | Current structured SVG contract plus complete `pptx_masters`/`pptx_layouts` mapping | Stop if structure cannot be restored without an explicit design decision |
...
|---|---|
| Explicit current layout/deck workspace root containing `templates/design_spec.md` | Enter main Step 3 template option; use the workspace root, not its inner `templates/` directory |
| Explicit legacy-flat layout/deck root containing `design_spec.md` | Enter main Step 3 through the compatibility reader; flat packaging alone does not require structure restoration |
| Explicit brand directory containing `design_spec.md` | Enter main Step 3 under the existing `create-brand` contract; brand packaging is unchanged by `create-template` workspace rules |
...
**Forbidden - fuzzy resolution**: Do not resolve bare names to local template directories on the user's behalf. The user must provide the path that enters Step 3. For a current layout/deck package, that path is the workspace root.
