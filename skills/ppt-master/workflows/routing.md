...
| Explicit template directory path | User provides a directory containing `design_spec.md` with `kind: brand`, `kind: layout`, or `kind: deck` | Main `SKILL.md` Step 3 | Fuzzy template lookup by bare name | Path resolves, frontmatter kind is valid, and every deck/layout SVG satisfies the current structured Master/Layout/slot contract | External assets are copied/fused into `<project>/templates/`; the same project's own `templates/` path is consumed in place | Route a legacy package to [`restore-pptx-structure`](./restore-pptx-structure.md) before Step 3; stop if the path does not exist or lacks valid `design_spec.md` |
