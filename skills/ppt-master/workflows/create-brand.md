...
| Brand site URL | `uvx ppt-master web-to-md <URL>`, then `Read` the result | color references, font references, voice/tone |
| Branded PPTX | `uvx ppt-master ppt-to-md <file>`, then read theme XML | colors, typography (literal) |
| Brand PDF | `uvx ppt-master pdf-to-md <file>` | voice/tone; sometimes color/font references |
...
```bash
uvx ppt-master register-template --kind brand <brand_id> --dry-run
```

Fix every reported error, then register the same workspace:

```bash
uvx ppt-master register-template --kind brand <brand_id>
```
...
- To edit: modify `templates/brands/<brand_id>/templates/design_spec.md`, then re-run `uvx ppt-master register-template --kind brand <brand_id>`
```
