...
```bash
uvx ppt-master svg-authoring-view "<import_workspace>/authoring-svg" --refresh-summary
```
...
```bash
uvx ppt-master source-to-md "<file_or_URL_or_dir>" -o "<text_analysis_workspace>"
```
...
```bash
uvx ppt-master register-template <brand_id> --kind brand --dry-run
```
...
```bash
uvx ppt-master register-template <style_id> --kind style --dry-run
```
...
uvx ppt-master svg-quality-check "<template_workspace>/templates" --template-mode --format <canvas_format>
```
...
```bash
uvx ppt-master template-preview-pptx "<template_workspace>" --force
```
...
```bash
# For brand
uvx ppt-master register-template <template_id> --kind brand

# For style
uvx ppt-master register-template <template_id> --kind style
...
>
> ```bash
> uvx ppt-master register-template --kind style --rebuild-all
