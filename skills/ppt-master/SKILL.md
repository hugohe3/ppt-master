...
2. In `build_slides.py`: append 5 new `page_NN_*()` functions and add their entries to the `pages` dict in `generate()`
...
After all batches succeed, all SVGs are in `svg_output/`.

> ⚠️ **Source of truth rule**: After Step 6.A, `build_slides.py` is the source of truth for SVGs. Hand-editing individual SVG files in `svg_output/` will be overwritten on the next run. All visual changes must go through `build_slides.py`.

#### Step 6.C — Speaker Notes Pass

After all SVGs are on disk, hand-author `notes/total.md` in one pass following the format and language rules in [executor-base.md](references/executor-base.md) §8. Notes are a plain Markdown document — they are **not** generated by `build_slides.py`.
...
- [x] Speaker notes hand-authored at notes/total.md
```
