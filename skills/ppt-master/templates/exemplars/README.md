...
| 文件 | 类型 | 来源 | 为什么它高级（认可点）|
|------|------|------|---------------------|
| `cover_image-canvas-keynote.svg` | 封面 | 通用范例（占位文案）| 满版 hero 大图 + 大字渐变标题 + 眉标 chip + 底部磨砂特征胶囊；keynote 发布会感，文字仍为可编辑 SVG 叠层 |

> 注：示例中的品牌色 / 文案均为通用占位；复用时按目标 deck 的 `spec_lock` 替换，保留结构 / 组件 / 排版工艺。
