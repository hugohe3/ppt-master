...
> - **native-diagram** rows — `path` = `templates/native_diagrams/<key>`; `summary-quote` verbatim from `diagrams_index.json` `diagrams.<key>.pick`. (Native picks are gated on the deck welcoming dimensional figures — see `strategist.md` Per-page Vehicle Decision.)
