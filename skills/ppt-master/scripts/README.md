...
`pptx_to_svg.py` annotates supported unmerged tables and conservative classic-chart caches with `data-pptx-native` metadata. Table metadata records the source table-style ID and supported direct solid cell fills/basic text formatting, but the SVG fallback currently resolves direct cell properties only; inherited built-in/custom table-style appearance is not guaranteed. Chart title/legend/axis titles and plot-level data-label flags for area/bar/column/line charts are retained when the current schema can represent them. Tables with direct borders, non-solid fills, mixed rich-text formatting, or merges remain fallback-only, as do charts with unsupported label scopes/types, custom axis semantics, trendlines/error bars, or subtype options. Fallback-only objects keep rendered SVG content or a baked chart preview and carry `data-pptx-native-status`, which validation and `--native-objects` export report as a warning. A chart with no baked preview instead carries `data-pptx-visual-status="placeholder"` plus `data-pptx-route-status="reconstruction-only"`; validation/export warn, default export keeps the placeholder, and `--native-objects` may reconstruct it when a valid active native-chart payload is present.
