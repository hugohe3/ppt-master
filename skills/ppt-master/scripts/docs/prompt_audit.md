...
- **Budget ceilings** (`budget_policy: fixed_upper_bound`): budgets are stable, deliberately rounded limits rather than mirrors of the current token count. Establish a ceiling with roughly 10% working headroom and round it up in 250-token increments below 10k, 1k increments below 100k, or 5k increments from 100k upward; the manifest loader enforces those increments. Ordinary wording edits do not change the ceiling. Raise it only when a deliberate architecture-level scope expansion would otherwise cross it, then choose the next rounded limit with comparable headroom and record why in that change.
