...
**Stage-2 catalog read gate.** Strategist authors the three whole solutions with no mode/style/rendering index or detail file in context, then opens `modes/_index.md`, `visual-styles/_index.md`, and `image-renderings/_index.md` once only to record genuinely matching bases. No detail file is read before final confirmation; the selected direction's referenced files are read while authoring the Design Spec. `custom` has no required catalog relationship: one exact preset may supply an entire editable projection unchanged, several may contribute distinct jobs, or no preset may be used. These are examples, not a closed classification. The behavior remains non-empty; omit every source whose contribution it cannot state and never add a decorative second basis. Unselected sibling files never enter context.
