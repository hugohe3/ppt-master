...
- **Enumerable + custom** — canvas / icons retain blank manual inputs. Mode and visual style first show the three project-specific values projected from the complete directions, then the full base catalog. Any projected `custom` variant keeps its own visible identity and behavior even though a selected result writes literal `custom` plus its behavior sibling.
...
- **Coordinated generative directions** — `design_directions` carries exactly three complete candidates authored top-down from the project contract. Each has a unique stable id and bundles mode, visual style, color, typography, icon id, and generated-image rendering regardless of recommended image source. All three must be viable and meaningfully distinguishable as whole solutions; they do not need different catalog ids or forced safe / shifted / bold archetypes. The page can still render legacy top-level `color`, `typography`, and `image_strategy` candidates, but new staged recommendations use the coordinated bundle.

Direction-local custom projections apply to mode, visual style, and generated-image rendering; a selected custom value cannot be blank. Legacy standalone `custom_candidates` remain readable but are optional and are not authored in new files. Color / typography keep their existing manual Custom cards. Image usage uses source ids plus `image_notes`; closed sets have no Custom path.
...
The example shows one candidate's complete shape; the actual array repeats that shape for exactly three candidates. Every candidate requires a unique stable `id`, mode, visual style, icon value, complete six-role palette, complete heading/body stack, and complete image strategy even when `recommend.image_usage` excludes AI. Custom mode/style/rendering values require their localized behavior sibling. Legacy grids remain readable only with three complete palettes and complete typography.
...
- `custom_candidates` is an optional legacy recommendation-only shape. New files place every project-specific preset or custom variant inside one of the three `design_directions`; this preserves three distinct UI identities even when several variants serialize as `custom`. A custom behavior names exact catalog ids only when it actually uses them as synthesis bases. The UI rejects a selected blank and omits unselected candidates from `result.json`. Template-backed variants obey inherited identity, prototype capacity, and `template_application`.
...
- **Generated-image direction** appears only for current `image_usage: ai`, but all three project candidates already exist in `design_directions` before that toggle. Turning AI on reveals those candidates immediately without a backend rerun, followed by the 20 system preset styles. Direction-local custom variants remain separately identifiable and editable; a preset submits its id, while Custom submits `rendering: "custom"` + non-empty `behavior`. Turning AI off omits `image_strategy` from the final result without deleting the authored recommendation candidates. Catalog-based custom behavior names exact ids for optional `image_rendering_references`; a novel behavior has none. The left preview follows selection. No image palette is written; deck colors remain authoritative, and legacy `image_strategy.palette` is ignored.
- **`design_directions`** is the canonical Stage-2 starting set: exactly three top-down, project-fit bundles with stable ids, localized copy, mode, style, icons, always-present image strategy, complete language-aware typography, and HEX `background`, `secondary_bg`, `primary`, `accent`, `secondary_accent`, `body_text`. Clicking a card applies every field it owns; lower controls may then diverge and the card shows an adjusted state. Clicking the same or another card reapplies that immutable bundle. `result.json` stores the current flattened components, never a direction id.
