...
- **Closed enumerable** — illustration usage / formula policy / generation mode / refine spec, plus AI source only when image usage may include `ai`. These have no Custom box; out-of-catalog values snap back to the recommended option. Use pipeline vocabulary: icon ids are actual library ids such as `tabler-outline`, or `emoji` for system emoji; image usage labels mirror Strategist terminology: `ai` = AI-generated, `web` = Web-sourced, `provided` = User-provided, `placeholder` = Placeholder, `none` = No images. Use image-usage custom prose only when several sources are mixed. `illustration_usage` is a closed two-option intent toggle (`none` / `use`) — how heavily illustrations are used and where they are sourced are decided downstream by Strategist + `image_usage`, not here.
...
**Custom box** appears only on fields whose universe is genuinely open — `canvas`, `mode`, `visual_style`, `icons`, and `image_usage`. Fully closed sets — `illustration_usage`, `image_ai_path`, `formula_policy`, `generation_mode`, `refine_spec` — have **no** Custom box; an out-of-catalog value there is snapped back to the recommended option.
...
| `2` | realization — page count, color, typography, icons, formula, image usage + illustration usage + strategy, generation mode, refine spec | **Confirm** | writes `result.json` `{ stage: "final", status: "confirmed", <all fields> }`, then shuts the page down |
...
- `recommend.illustration_usage` is the user's illustration intent toggle inside the image section, not a second image-source enum. It is a closed two-option field — `none` (don't intentionally add illustrations) or `use` (make illustrations part of the deck). How heavily (sparse vs pervasive) and from where are decided downstream by Strategist + `image_usage`, which maps to §VIII rows using the normal source vocabulary (`ai` / `web` / `user` / `placeholder` / `slice`).
...
- `illustration_usage` is a closed `none` / `use` toggle. `none` = do not intentionally add decorative illustrations (default, current behavior); `use` = make illustrations part of the deck, with intensity (sparse vs pervasive) the Strategist's call from content + visual style. The source still comes from `image_usage`: `image_usage: none` produces no decorative illustration rows; otherwise AI-backed illustration sets may use an illustration sheet plus `slice` rows, provided assets become `user` rows, web assets become `web` rows when licensing is acceptable, and deferred illustrations become `placeholder` rows.
