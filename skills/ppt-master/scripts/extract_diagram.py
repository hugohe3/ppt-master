...
def main() -> int:
    ...
    ap.add_argument("--structure", default="",
                    help="One-line note on the figure's slot structure (authoring hint in meta.json)")
    args = ap.parse_args()

    out = Path(args.output) if args.output else Path(f"native_diagram_{args.slide_num}")
    meta = extract_diagram(args.source, args.slide_num, out, key=args.key,
                           summary=args.summary, structure=args.structure)
