...
def _apply_audio(
    ...
    *,
    ...
    if not poster_path.exists():
        poster_path.write_bytes(_get_audio_poster_bytes())
    ...
    if duration is None:
        print(f"  [warn] Cannot read duration for {audio_path.name}, using default 5.0s", file=sys.stderr)
        duration = 5.0
