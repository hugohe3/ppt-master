...
rect in the SVG::
    ...
    <rect data-native-diagram="combo_product_system"
          ...
          data-text='{"0":"My title","1":"My label"}'
          x="140" y="120" width="1000" height="480" fill="none"/>

and at conversion time the named component's shapes (already DrawingML) are
spliced in, scaled to the placeholder rect, optionally recolored + text-filled,
and (with ``data-font``) re-fonted to the deck typeface. Set ``data-font`` to the
...
def _parse_recolor(spec: str | None) -> dict:
    ...
    Each side is normalised to a bare 6-digit uppercase hex; a malformed pair is
    skipped with a warning instead of being injected as an invalid color value.
    """
    out: dict[str, str] = {}
    for pair in (spec or "").split(","):
        ...
        if _HEX_RE.fullmatch(old) and _HEX_RE.fullmatch(new):
            out[old] = new
        elif pair.strip():
            print(
                f"Warning: data-recolor skipped invalid pair {pair.strip()!r} "
                f"(want SRChex=DSThex, 6 hex digits each)",
                file=sys.stderr,
            )
    return out
...
def _apply_recolor(xml: str, cmap: dict) -> str:
    """Remap ``<a:srgbClr val="OLD">`` -> NEW in a SINGLE pass.

    One regex over every source hex (resolved through *cmap*) avoids the
    chained-substitution bug a per-pair loop has: a NEW color equal to some other
    OLD color would otherwise be re-matched and corrupted by a later pass. Only
    explicit ``srgbClr`` hexes are remapped; ``schemeClr`` theme refs are left
    untouched (components are theme-flattened, so their colors are ``srgbClr``).
    """
    if not cmap:
        return xml
    alternation = "|".join(re.escape(k) for k in cmap)
    pat = re.compile(r'(<a:srgbClr val=")(' + alternation + r')(")', re.IGNORECASE)
    return pat.sub(lambda m: m.group(1) + cmap[m.group(2).upper()] + m.group(3), xml)
...
def _apply_text_fills(xml: str, data_text: str | None) -> str:
    ...
    ``data-text`` maps slot id (the run's index in document order, as recorded in
    ``meta.text_slots``) to replacement text. Two shapes are accepted:
    ``{"<id>": "<text>"}`` (object) or ``[{"id": <n>, "text": "<text>"}, ...]``
    (array; ``"slot"`` is also accepted as the id key). Slots not provided keep
    their original (source) text. A no-op — bad JSON, an empty map, or zero slots
    matched — is **warned to stderr**: silently keeping the source deck's text is
    the failure that looks like the feature ran when it did not.
    """
    if not data_text:
        return xml
    try:
        ...
        print("[native-diagram] data-text is not valid JSON; diagram text left unchanged", file=sys.stderr)
        return xml
    if isinstance(parsed, list):
        fills = {
            str(e.get("id", e.get("slot"))): e["text"]
            for e in parsed
            if isinstance(e, dict) and "text" in e and (e.get("id") is not None or e.get("slot") is not None)
        }
    elif isinstance(parsed, dict):
        fills = {str(k): v for k, v in parsed.items()}
    else:
        fills = {}
    if not fills:
        print("[native-diagram] data-text produced no slot map; diagram text left unchanged", file=sys.stderr)
        return xml
    counter = [-1]
    used = [0]
    ...
    out = _AT_RE.sub(repl, xml)
    if used[0] == 0:
        print("[native-diagram] data-text matched 0 of the diagram's text slots "
              "(slot-id range mismatch?); original text kept", file=sys.stderr)
    return out
...
# Strip an existing typeface child, whether self-closing (<a:latin .../>) or
# paired (<a:latin ...>...</a:latin>, as PowerPoint emits when it carries extLst).
_FONT_CHILD_RE = re.compile(
    r"<a:(?:latin|ea|cs)\b[^>]*/>"
    r"|<a:(?:latin|ea|cs)\b[^>]*>.*?</a:(?:latin|ea|cs)>",
    re.DOTALL,
)

_GENERIC_FAMILIES = {"sans-serif", "serif", "monospace", "cursive", "fantasy", "system-ui"}


def _first_family(font: str) -> str:
    """Reduce a CSS font stack to a single DrawingML typeface.

    ``spec_lock typography.font_family`` is a CSS stack
    (``"Microsoft YaHei", Arial, sans-serif``), but DrawingML's ``typeface``
    attribute accepts only ONE font name — feeding it the whole stack produces a
    malformed PPTX. Take the first concrete family, stripping quotes/whitespace
    and skipping the generic CSS fallbacks.
    """
    for part in font.split(","):
        name = part.strip().strip('"').strip("'").strip()
        if name and name.lower() not in _GENERIC_FAMILIES:
            return name
    return font.strip().strip('"').strip("'").strip()
...
def _apply_font(xml: str, font: str | None) -> str:
    ...
    ``latin``/``ea``/``cs`` are replaced and missing ones injected so every run
    (and ``endParaRPr``/``defRPr``) renders in the deck's typeface.
    """
    if not font:
        return xml
    face = _first_family(font)
    if not face:
        return xml
    fonts = (
        f'<a:latin typeface="{face}"/>'
        f'<a:ea typeface="{face}"/>'
        f'<a:cs typeface="{face}"/>'
    )
