...
| Any selected `family/key` visualization reference, or a current-page legacy `page_charts` row | [`executor-visualization.md`](./executor-visualization.md), then the resolver-returned family branch below |
...
| Any page-local qualitative topology, including a structure-family reference | [`executor-structure.md`](./executor-structure.md) |
...
Evaluate family branches from each object's actual information model, not only from the primary reference. A family selects construction guidance but never native readiness; `Native-ready` is an independent object-level decision. Page-local `structure` also never implies package-level `pptx_structure.mode: structured`.
...
**Hard rule — binding selection vs realization**: use Strategist-selected semantic content, resources/paths, visualization `family/key` references, template/layout routing keys, core fonts, palette anchors, icon-library/stroke anchors, and crop boundaries. Adapt realization—including which prepared project-local icon, if any, best serves each page—without changing those binding selections, except sparse local font/color garnish allowed below. A §VIII preferred image pattern is not a template/layout routing key; [`executor-image.md`](./executor-image.md) owns its realization freedom. Missing or unresolved material stops execution and returns to Strategist-owned acquisition/failure recovery; never search, generate, download, sync, invent, or substitute it. Binding selection changes require upstream repair.
...
- **Inherited containers**: preserve meaningful template frames; restyle radius, fill, stroke, and depth from the active Design Spec and `spec_lock.md`. Selected catalog-reference adaptation is owned by [`executor-visualization.md`](./executor-visualization.md); preview effects never override project styling or structural roles.
