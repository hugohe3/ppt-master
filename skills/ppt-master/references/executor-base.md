...
> Narrative skeleton and visual aesthetic come from the locked values applied against the complete [`modes/`](./modes/_index.md) and [`visual-styles/`](./visual-styles/_index.md) catalogs already loaded for this Executor context. Optional `*_references` identify actual custom synthesis bases; they are not load triggers. [`shared-standards-core.md`](./shared-standards-core.md) supplies the technical boundary plus the fallback visual-quality and leading defaults when those authorities are silent.
