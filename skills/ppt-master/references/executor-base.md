...
- **Generation flow — read [build-slides-spec.md](build-slides-spec.md)**: All SVG generation goes through a single Python file `<project_path>/build_slides.py` that is the source of truth. The flow is:
  ...
  3. **Step 6.C — Speaker notes pass**: After all SVGs are on disk, hand-author `notes/total.md` in one go (see §8). Notes are **not** generated by `build_slides.py` — they are a separate Markdown document so prose text doesn't get tangled in two-layer Python/SVG quoting
...
After **all SVG pages are generated and finalized** (Step 6.A and 6.B done), enter the "Logic Construction Phase" and hand-author the complete speaker notes document directly as Markdown at `notes/total.md`. This is **Step 6.C** in the executor workflow.

Speaker notes are **not** produced by `build_slides.py`. Keeping prose out of the .py file avoids two-layer Python/SVG quoting on narrative text and lets the executor write notes with the finished deck in view.

**Why write notes in one pass after the SVGs land?** Batch-writing notes allows planning transitions like a script, ensuring coherent presentation logic across the full deck. Writing them per-page while pages are still mid-iteration leads to transitions that no longer make sense once later pages are finalized.

**Required Markdown format**: Each page starts with `# <number>_<page_title>`, separated by `---` between pages. Each page includes: script text (2-5 sentences), `Key points: ① ② ③`, `Duration: X minutes`. Except for the first page, each page's text starts with a `[Transition]` phrase.
