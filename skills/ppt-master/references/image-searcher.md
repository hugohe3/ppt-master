...
```bash
uvx ppt-master image-search "<query>" \
  --filename <name>.jpg \
  --slide <slide_id> \
...
```bash
uvx ppt-master image-search --batch <project_path>/images/image_queries.json \
  -o <project_path>/images \
  --save-candidates
```
...
5. **manual URL replace (universal, model-agnostic)** — use a directly selected URL and swap it in:
   ```bash
   uvx ppt-master image-search --from-url <image-url> --filename <name>.jpg -o <project_path>/images
   ```
...
```bash
uvx ppt-master image-search "<query>" --filename <name>.jpg -o <project_path>/images \
  --save-candidates
```
...
```bash
uvx ppt-master image-search --promote candidate_03.jpg --filename <name>.jpg -o <project_path>/images

# No pass on page 1, but candidates.json says has_more_candidates: true
uvx ppt-master image-search "<same query>" --filename <name>.jpg \
...
uvx ppt-master image-search --promote candidate_03.jpg --filename <name>.jpg \
