...
> **Default presentation surface — Confirm UI.** Before the first confirmation phase, apply the sticky per-run surface decision in [`confirm-surface.md`](./confirm-surface.md) and author the Stage-1 and Stage-2 payloads in the shapes it defines; the chat/delegated branch keeps equivalent state without fabricating UI receipts. Stage 1 writes canonical BCP-47 `primary_language`; Stage 2 carries exactly three immutable `design_directions`, and the final result stores only the current component values, never a direction id as execution authority. Server lifecycle and the template-selection sidecar: [`confirm_ui.md`](../scripts/docs/confirm_ui.md).
...
> 4. Put known basenames in the final batch; search an uncertain one only inside the chosen library (or `simple-icons` for a real brand mark) by the drawable object, never the abstract concept — [`../templates/icons/README.md`](../templates/icons/README.md) § Searching for Icons.
> 5. Copy and validate in one batch: `python3 skills/ppt-master/scripts/icon_sync.py <project_path> <lib/name> [<lib/name> …]` materializes `<project>/icons/<lib>/`; keep each successful case-sensitive `lib/name` (bundled basenames are lowercase).
> 6. Record each synced path with broad suitable scenarios in `design_spec.md` §VI and the same curated pool, primary library, and any stroke-library `stroke_width` in `spec_lock.md icons`; `simple-icons/*` ids join that inventory without becoming a second library. `inventory` indexes the synced pool; other prepared project-local icons remain usable.
>
...
The three indexes are the only basis selectors: freeze each direction's bases from them, then read only the deduplicated referenced detail files; Executor later reads one locked preset file or a custom's exact references (see [`generate-pptx`](../workflows/generate-pptx.md) Step 6).
...
> Note: §IX is the page brief at the confirmed depth; Executor retains it with the lock until context invalidation, then reloads both once.
...
**Execution lock content**: `spec_lock.md` carries communication, stable color/type anchors, icons, images, page rhythm, Chart/Table references, and route-specific PowerPoint structure; qualitative relationships stay only in §IX. Its section grammar, typography projection (`title_family` + `body_family` + every `<role>_family` and size anchor), `page_visualizations`, and the flat/structured `pptx_structure` sections are defined in [`spec_lock_reference.md`](../templates/spec_lock_reference.md) §2–4 — never re-derive a confirmed anchor, collapse distinct stacks into `font_family`, or drop a recurring role. Executor context policy lives in [executor-base.md](executor-base.md) §2.1. Repair from Gate 2's active decision authority, then re-author affected lock rows.
...
**Contextual extension**: derived paint or sparse local font/color garnish may stay in one SVG while non-structural and non-recurring. New base/semantic colors, structural/recurring fonts, resources, or recurring cross-page identity patterns require upstream repair; a page-local §VIII preferred image pattern follows [`executor-image.md`](./executor-image.md) and may change during realization. Executor never reverse-projects a local choice as planning fact. Promote recurring garnish upstream before reuse, read back and validate the affected planning fragments, and never add values to silence a comparison.
   ...
   - **Custom behavior is concise and executable**: for confirmed `custom` mode or visual style, project one resolved `mode_behavior` / `visual_style_behavior` sentence or short paragraph plus the exact `*_references` only when catalog entries are actually used; omit selection history and provenance.
   ...
   - **pptx_structure and page_visualizations**: free-design, brand-only, and `template_reuse_scope: style` routes write `mode: flat` and omit every structured mapping section; `mirror|layout` writes `mode: structured` with `template_adherence` and the four mapping sections under [`strategist-template.md`](./strategist-template.md). Project at most one §VII `P<NN>: <chart|table>/<key>` per page; usage, children, and qualitative relationships stay in the Design Spec. Grammar: [`spec_lock_reference.md`](../templates/spec_lock_reference.md) §3–4.
