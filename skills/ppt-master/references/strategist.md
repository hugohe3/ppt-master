...
- User/template typography wins. Otherwise ≥3 Stage-2 directions include safe concord and purposeful contrast; no extra font round or near-duplicate pair.
- A category may explain candidate `note`, never replace the choice. Before Stage 2, resolve concrete `heading` / `body` `cjk`, `latin`, `css`, and positive `body_size`; `name` labels the pair and Custom fills all four faces. Repeat fixed stacks.
- Resolve by language/target: installed/approved faces may lead; otherwise use a safe face and express character via scale, weight, spacing, color, composition. Fonts are not embedded; unavailable options are post-export Typography upgrade References after target installation, never stacks/lock.
- Avoid near-equivalent roles such as YaHei↔PingFang, SimSun↔Songti, Arial↔Helvetica↔Segoe UI, or Times New Roman↔Times. Preview counterparts/CSS tails are not deterministic PowerPoint fallbacks.
- **Default — language-matched safe concord (unless identity is confirmed)**: Include one approved heading/body stack—zh-CN `Microsoft YaHei` + `Arial`; zh-TW `Microsoft JhengHei` + `Arial`; ja `Meiryo` + `Arial`; ko `Malgun Gothic` + `Arial`; Latin `Arial` plus locale EA when needed. This is one candidate, not a mandate.
- **Reference — not a constraint**: Internal recall, never Confirm UI options: neutral sans, humanist sans, editorial serif, display/expressive, mono/data. Map neutral/humanist → safe sans; editorial → safe serif; display → approved face or treated safe sans/serif; mono/data → safe Latin mono + EA companion. Use display mainly for covers/dividers/short titles and mono/data for code/data/coordinates/short labels.
