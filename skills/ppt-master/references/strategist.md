...
> 6. **Copy each chosen icon into the project as you confirm it** — `uvx ppt-master icon-sync <project_path> <lib/name> [<lib/name> …]`. This populates `<project>/icons/<lib>/` (the set the Executor embeds from) and, more importantly, **validates existence on the spot**.
