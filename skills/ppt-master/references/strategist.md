...
> **Top-down direction construction.** Author the three complete solution directions before selecting their component values; do not assemble three apparent solutions afterward from independent mode/style/color recommendations. Each direction may use presets, `custom`, or repeated fixed values. A `custom` component carries visible, non-empty behavior prose and exact references only when it actually synthesizes catalog material. The three direction projections are the project-specific component candidates shown above the complete base catalogs; they are not a separate fourth Custom proposal. Never write bespoke prose as the enum value.

#### Layer 1 — Communication mode

🚧 **GATE**: read [`modes/_index.md`](./modes/_index.md) plus all five sibling mode files once as one deterministic, boundary-preserving batch before authoring the three directions.
...
- A bespoke direction the five don't give — a nameable cadence (dialectic 正反合, myth-vs-reality, countdown, Socratic), a multi-act fusion of modes, or the user's own feel (confrontational here, detached there). Either the user asks, **or you recommend it** when a fusion / bespoke direction genuinely serves the deck better than a single preset (a recommendation the user confirms, like every lock). The *kind* doesn't matter → `mode: custom` + a `mode_behavior:` paragraph that **crystallizes the intent** (act sequence or posture shifts, title voice, page rhythm, register) concretely enough for the Executor to follow per page; it reads only `spec_lock.md`, never the chat. If the direction uses existing modes, retain those exact ids as its catalog basis; if it is genuinely new, do not invent a basis. One deck locks **one** value — a fusion is one `custom` describing the acts, never several modes. Avoid only the *dodge*: don't default to `custom` when a preset genuinely fits, and prefer a dominant mode + page-level variation when one mode leads.
- No user structure or cadence → derive each whole solution from the confirmed `communication_intent`, `audience_outcome`, source texture, and delivery context, then project its mode. Composite intent does not automatically require `custom`: choose the dominant spine of the body pages when one exists; use a concrete `custom` act sequence only when no single spine can serve the stated priority / sequence. The three whole solutions may share a mode when that is honestly best; distinguish them through the fields that materially change the outcome instead of forcing different mode ids.

Record the confirmed mode and rationale in `design_spec.md` first, including the exact catalog basis when a selected custom uses one. Then project `- mode:` to `spec_lock.md`; for `custom`, also project `- mode_behavior:` and, only when catalog material is actually used, `- mode_references: <id>, <id>`. Executor has the complete mode catalog in its execution context; references preserve provenance and synthesis intent, while an unreferenced novel custom follows the behavior directly.

#### Layer 2 — Visual style

🚧 **GATE**: read [`visual-styles/_index.md`](./visual-styles/_index.md) plus all 18 sibling style files once as one deterministic, boundary-preserving batch before authoring the three directions.
...
**Source**:
- User named a style (chat / template / beautify) → it is truth: map to the closest preset (or `custom` with a `visual_style_behavior` paragraph) and repeat that fixed value across all three directions. Keep the three whole solutions meaningful by varying only fields the user left open.
- No user description → author three project-fit whole solutions first, then project each solution's complete visual aesthetic. A projected value may be one preset or a `custom` synthesis; the directions may share catalog bases when their overall systems still differ meaningfully. Do not force different style ids, a safe-to-bold ladder, or one deliberately extreme option merely to manufacture variety. Give each direction a localized name and concise real-world outcome note; the Confirm UI derives three project-specific style candidates from those directions and then exposes all 18 base styles for manual selection.

**Forbidden — a non-catalog name as `visual_style`**: the value MUST be an `id` from the visual-styles catalog or literal `custom`; bespoke prose belongs only in `visual_style_behavior`. A name that is **not** in that catalog is not a visual style — most often it is an image-rendering name from the `_index` "Paired rendering" column (`flat`, `vector-illustration`, `digital-dashboard`, `3d-isometric`, `corporate-photo`, …), which names the §h *illustration* family, not the deck's layout aesthetic. Do not borrow it. (Names that are intentionally **both** a style and its paired rendering — `glassmorphism`, `blueprint`, `editorial`, `dark-tech` — are valid styles because they *are* in the catalog.) Generic baseline words — `flat` / flat-design / 扁平 / modern / clean / simple / minimal — are **not** custom-worthy either: the whole system is flat by default (shadows discouraged), so map them to the closest preset (flat + grid → `swiss-minimal`; flat + rounded → `soft-rounded`; flat + dense → `brutalist`). Reserve a custom lock for an aesthetic no preset covers.
...
🚧 **GATE**: The complete visual-style catalog is already loaded. Apply the projected preset's §2 Typography character, or resolve a custom synthesis under `visual_style_behavior` and its exact `visual_style_references`. The title carries the character; the body may remain neutral.
...
1. Before authoring Stage-2 directions, read [`strategist-image.md`](./strategist-image.md) plus [`image-renderings/_index.md`](./image-renderings/_index.md) and all 20 sibling rendering files once as one deterministic, boundary-preserving batch. Author one complete rendering candidate inside each direction before deciding whether `recommend.image_usage` includes AI.
...
Strategist reads each index plus every sibling entry once as a deterministic batch before authoring the three complete directions. Executor independently reads both complete catalogs once in its execution context, then applies the confirmed value; `*_references` record only the actual basis of a custom synthesis (see [`generate-pptx`](../workflows/generate-pptx.md) Step 6).
