...
4. **Generate execution lock**: read `templates/spec_lock_reference.md` and produce `projects/<project_name>.../spec_lock.md` — a distilled, machine-readable short form of the color / typography / icon / image / **page_rhythm** / **pptx_layouts** / **page_layouts** / **page_charts** decisions above. This file is what the Executor re-reads before every page (see [executor-base.md](executor-base.md) §2.1). The values in `spec_lock.md` MUST exactly match the decisions recorded in `design_spec.md`; if they ever diverge, `spec_lock.md` wins and `design_spec.md` should be treated as historical narrative.
   ...
   - **pptx_layouts (native output Layout contract)**: Write one `P<NN>: <layout_key> | <PowerPoint layout name>` row for every page in every new generated deck. Free-design and brand-only routes remain `mode: baseline`; derive stable keys and picker names from the compositions planned in §IX, without adding `page_layouts`. Name each key after its composition (`timeline-spine`, `kpi-band-trio`), never after a PowerPoint stock role. Template strict uses the selected template SVG's exact key/name; adaptive may introduce a new stable key/name. Legacy preserve uses the exact source key/name. A legacy baseline project whose whole section is absent remains valid and uses page-role compatibility behavior.
   - **Layout-family boundary**: Different reusable composition → different key; a free-design deck of visually distinct pages yields close to one key per page. Reuse a key only when pages share the same reusable static Layout layer and the same placeholder type/index/bounds contract; do not split keys merely because slide-local wording, data, or imagery differs. Decide these families from the authored page plan — never by post-hoc visual clustering, filenames, or content similarity. Keep concrete slide content and complex grouped compositions Slide-local; they are not Layout material. **Self-check before writing the section**: count distinct keys against the distinct compositions in §IX — a deck of mostly distinct pages collapsing to a handful of role keys (`cover` / `title-content` / `section`) means pages were clustered by role, not composition; re-derive.
   - **page_layouts (deck/layout templates only)**: Add one `P<NN>: <svg_basename>` row for every page. Strict inherits that SVG contract unchanged. Adaptive uses it as the architecture reference and may evolve the Layout layer under a new `pptx_layouts` key; it must not drop into free design. Omit the whole section for free-design and brand-only routes; their required `pptx_layouts` rows do not imply an SVG template reference.
