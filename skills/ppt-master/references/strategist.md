...
**Native template contract**: When `<project_path>/templates/native_structure.json` exists, read it before assigning page layouts. It is the factual source for PowerPoint layout keys/names, parent masters, and placeholder identities; the template SVG roster remains the visual/content-fit source.
...
|---|---|
| `strict` | Map every generated page to one actual template SVG in `page_layouts`. When the selected structure segment also ships a validated native pair, use `pptx_structure.mode: preserve` and map every page to that SVG's exact source layout key/name. |
| `adaptive` | Treat the template roster as optional visual/layout references. Write `page_layouts` only where a roster page fits; leave other pages free. Keep `pptx_structure.mode: baseline` even when the native pair exists. |
...
4. **Generate execution lock**: read `templates/spec_lock_reference.md` and produce `projects/<project_name>.../spec_lock.md` — a distilled, machine-readable short form of the color / typography / icon / image / **page_rhythm** / **page_layouts** / **page_charts** decisions above. This file is what the Executor re-reads before every page (see [executor-base.md](executor-base.md) §2.1). The values in `spec_lock.md` MUST exactly match the decisions recorded in `design_spec.md`; if they ever diverge, `spec_lock.md` wins and `design_spec.md` should be treated as historical narrative.
   ...
   - **pptx_structure is mandatory**: Write `mode: baseline` by default. When a deck/layout template was confirmed, also write `template_adherence: strict|adaptive`. Use `mode: preserve` only when adherence is `strict` and the selected template structure segment ships both `templates/native_structure.json` and `templates/source_template.pptx`; add project-relative `source_template` and `native_structure` rows exactly as shown in `spec_lock_reference.md`. `adaptive` always stays on `baseline`, regardless of whether the pair exists. Use `mode: template` only when the user explicitly requests a reusable PowerPoint Master/Layout/Placeholder deliverable and no preserved native pair owns the structure. Do not choose `flat`; it is a CLI-only diagnostic override.
   - **pptx_layouts (template/preserve structure)**: When `pptx_structure.mode` is `template` or `preserve`, write one `P<NN>: <layout_key> | <PowerPoint layout name>` row for every page. In preserve mode, take the exact key/name from the chosen template SVG root and verify it exists in `native_structure.json`; never rename source layouts. In template mode, reuse a layout key when pages share the same static layout layer and placeholder contract; page content differences do not justify new keys. One-off cover/section layouts are valid. Omit the whole section in baseline mode.
   - **page_layouts (write only when a template is in use)**: For each page that inherits a template SVG, add `P<NN>: <svg_basename>` (e.g., `P04: 03a_content_image_text`). With `template_adherence: strict`, every generated page requires a row and the basename must exist in the selected roster. With `adaptive`, pages designed freely get **no entry** — Executor reads the absence as "free design, no inheritance". If zero pages use a template, omit the section entirely.
