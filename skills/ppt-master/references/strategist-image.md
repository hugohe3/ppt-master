...
Read [`image-renderings/_index.md`](./image-renderings/_index.md) plus all 20 sibling rendering files once as one deterministic, boundary-preserving batch before constructing Stage 2. First author exactly three complete, project-fit solution directions, then project one complete `image_strategy` into each direction regardless of `recommend.image_usage`. Each candidate carries localized `name`, `rendering`, `visual`, and `mood`; `custom` also carries non-empty localized `behavior`. Mood includes a recognizable real-world analogy. All three must credibly serve their owning whole solution, but they need not use different rendering ids or span artificial safe / shifted / bold extremes. Image colors always inherit that direction's deck HEX roles; never add an image palette or alter deck colors to rescue a rendering.

A direction may use a preset or a `custom` synthesis. If a custom candidate combines or borrows existing renderings, name every exact id in its visible behavior after comparing the complete catalog; if it is genuinely novel, name no catalog basis. Under a template it obeys inherited identity and application. Only a confirmed custom locks its edited behavior as `image_rendering_behavior`; when catalog material is actually used, also project the exact ids as `image_rendering_references`, otherwise omit that field. Unselected candidates remain recommendation-only. Do not write a separate fourth `custom_candidates.image_strategy`; ignore legacy `image_palette`.
