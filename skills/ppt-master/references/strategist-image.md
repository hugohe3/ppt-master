...
**Default — one coherent sheet per compatible treatment family (may override when aspect, detail, quality, or semantic needs differ)**: for lettering, partition exact strings by intended artistic treatment before cell geometry; batch each treatment's compatible marks into one sheet, use separate sheets for different treatments, and split one treatment only when its element needs conflict. Apply the same family test to AI-generated spots. For each sheet, plan one unplaced `ai` Illustration Sheet row plus one placed `slice` row per used element; only slice rows enter `spec_lock.md images`. State the treatment and intended placement shape family in the sheet reference. Lettering sheets use `text_policy: embedded`; authoritative title/chrome wording stays outside. [`image-generator.md`](./image-generator.md) §4.3 owns grid, ratio, slicing, and execution details. Final Stage 2 chooses the AI execution path under `image-generator.md` §7; do not pre-empt or re-pick it here.
