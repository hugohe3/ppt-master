...
**Mandatory**: write one concise, non-empty, executable `Layout pattern` per placed row, including formulas. Treat it as an adaptable page-realization preference: state the intended image/content or image/shape relationship when that relationship matters, while allowing a plain split, full bleed, contained formula region, or other restrained composition when it best serves the page. Optional ids from the already-read [`image-layout-patterns.md`](./image-layout-patterns.md) must be exact. Executor may adapt or replace the suggestion while preserving resource role, file/source, must-use status, crop boundary, content, and explicit user/template constraints; layout-only changes need no upstream rewrite.
